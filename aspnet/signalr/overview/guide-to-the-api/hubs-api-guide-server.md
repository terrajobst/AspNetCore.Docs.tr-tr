...
İstemcilerin Hub'ınıza bağlanmak için kullanacağı rota tanımlamak için arama `MapSignalR` uygulama başladığında yöntemi. `MapSignalR`olan bir [genişletme yöntemi](https://msdn.microsoft.com/en-us/library/vstudio/bb383977.aspx) için `OwinExtensions` sınıfı. Aşağıdaki örnek, OWIN başlangıç sınıfı kullanarak SignalR hub'ları yol tanımlamak gösterilmiştir.
