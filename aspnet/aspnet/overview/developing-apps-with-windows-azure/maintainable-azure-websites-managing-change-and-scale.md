...
- <span data-ttu-id="fd7b1-138">Microsoft Azure aboneliği</span><span class="sxs-lookup"><span data-stu-id="fd7b1-138">A Microsoft Azure subscription</span></span>
    ...
    - <span data-ttu-id="fd7b1-142">Üyeleri [Microsoft iş ortağı ağı](http://aka.ms/watk-mpn) Cloud Essentials programı ücretsiz olarak aylık Azure KREDİLERİ alırsınız</span><span class="sxs-lookup"><span data-stu-id="fd7b1-142">Members of the [Microsoft Partner Network](http://aka.ms/watk-mpn) Cloud Essentials program receive monthly Azure credits at no charge</span></span>
...
3. <span data-ttu-id="fd7b1-345">Tıklayın **Evet** değişiklikleri onaylamak için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-345">Click **Yes** to confirm the changes.</span></span>
    ...
    <span data-ttu-id="fd7b1-347">*Standart mod değişikliğini onaylanıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-347">*Confirming the change to Standard mode*</span></span>
...
1. <span data-ttu-id="fd7b1-401">Yönetim Portalı'na dönün ve aşamalandırma site yuvası seçin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-401">Go back to the management portal and select the staging site slot.</span></span> <span data-ttu-id="fd7b1-402">Tıklayın **takas** komut çubuğunda.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-402">Click **Swap** in the command bar.</span></span>

    ![Üretim için değiştirme](maintainable-azure-websites-managing-change-and-scale/_static/image44.png)

    <span data-ttu-id="fd7b1-404">*Üretim için değiştirme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-404">*Swap to production*</span></span>
2. <span data-ttu-id="fd7b1-405">Tıklayın **Evet** değiştirme işlemine devam etmek için onay iletişim kutusunda.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-405">Click **Yes** in the confirmation dialog box to proceed with the swap operation.</span></span> <span data-ttu-id="fd7b1-406">Azure, hazırlama sitesi içeriğini üretim sitesini içerikle hemen değiştireceksiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-406">Azure will immediately swap the content of the production site with the content of the staging site.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-407">Bazı ayarlar hazırlanmış sürümünden (örn: bağlantı dizesi geçersiz kılmalar, işleyici eşlemeleri, vb.) üretim sürümü için otomatik olarak kopyalanır, ancak diğer ayarları değiştirmez (örneğin DNS uç noktaları, SSL bağlamaları, vb.).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-407">Some settings from the staged version will automatically be copied to the production version (e.g. connection string overrides, handler mappings, etc.) but other settings will not change (e.g. DNS endpoints, SSL bindings, etc.).</span></span>

    ![Değiştirme işlemi onayı](maintainable-azure-websites-managing-change-and-scale/_static/image45.png)

    <span data-ttu-id="fd7b1-409">*Değiştirme işlemi onayı*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-409">*Confirming swap operation*</span></span>
3. <span data-ttu-id="fd7b1-410">Değiştirme işlemi tamamlandıktan sonra üretim yuvası seçip tıklayın **Gözat** üretim sitesini açmak için komut çubuğunda.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-410">Once the swap is complete, select the production slot and click **Browse** in the command bar to open the production site.</span></span> <span data-ttu-id="fd7b1-411">Adres çubuğundaki URL dikkat edin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-411">Notice the URL in the address bar.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-412">Önbelleği temizlemek için tarayıcınızı yenilemeniz gerekebilir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-412">You might need to refresh your browser to clear cache.</span></span> <span data-ttu-id="fd7b1-413">Internet Explorer'da tuşlarına basarak bunu yapabilirsiniz **CTRL + R**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-413">In Internet Explorer, you can do this by pressing **CTRL+R**.</span></span>

    ![Üretim ortamında çalışan web uygulaması](maintainable-azure-websites-managing-change-and-scale/_static/image46.png)
4. <span data-ttu-id="fd7b1-415">İçinde **GitBash** konsolunda, üretim yuvası hedeflemek için yerel bir Git deposu için Uzak URL'LERİNİ güncellemeleri.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-415">In the **GitBash** console, update the remote URL for the local Git repository to target the production slot.</span></span> <span data-ttu-id="fd7b1-416">Bunu yapmak için yer tutucuları, dağıtım kullanıcı adı ve web uygulamanızın adıyla değiştirerek aşağıdaki komutu çalıştırın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-416">To do this, run the following command replacing the placeholders with your deployment username and the name of your web app.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-417">Aşağıdaki alıştırmalarda basitliğinin yanı sıra Laboratuvar için hazırlama yerine üretim sitesi için değişiklikler gönderir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-417">In the following exercises, you will push changes to the production site instead of staging just for the simplicity of the lab.</span></span> <span data-ttu-id="fd7b1-418">Bir gerçek dünya senaryosunda, üretim için yükseltmeden önce hazırlama ortamına değişiklikleri doğrulamak için önerilir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-418">In a real-world scenario, it is recommended to verify the changes in the staging environment before promoting to production.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample13.cmd)]

<a id="Exercise3"></a>
### <a name="exercise-3-performing-deployment-rollback-in-production"></a><span data-ttu-id="fd7b1-419">Alıştırma 3: Dağıtımı geri alma, üretimde gerçekleştirme</span><span class="sxs-lookup"><span data-stu-id="fd7b1-419">Exercise 3: Performing Deployment Rollback in Production</span></span>

<span data-ttu-id="fd7b1-420">Burada sahip olmadığınız arasındaki hazırlama ve üretim, örneğin, sık erişimli değiştirme gerçekleştirmek için bir hazırlama yuvası ile çalışıyorsanız senaryo vardır **ücretsiz** veya **paylaşılan** modu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-420">There are scenarios where you do not have a staging slot to perform hot swap between staging and production, for example, if you are working with **Free** or **Shared** mode.</span></span> <span data-ttu-id="fd7b1-421">Bu senaryolarda, uygulamanız bir sınama ortamında – yerel veya uzak bir sitedeki – üretim ortamına dağıtmadan önce test etmelisiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-421">In those scenarios, you should test your application in a testing environment –either locally or in a remote site– before deploying to production.</span></span> <span data-ttu-id="fd7b1-422">Ancak, sınama aşamasında algılanmayan sorunu üretim sitede kaynaklanabilecek mümkündür.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-422">However, it is possible that an issue not detected during the testing phase may arise in the production site.</span></span> <span data-ttu-id="fd7b1-423">Bu durumda, mümkün olan en kısa sürede uygulamanın önceki ve daha kararlı sürümü arasında kolayca geçiş için bir mekanizma olması önemlidir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-423">In this case, it is important to have a mechanism to easily switch to a previous and more stable version of the application as quickly as possible.</span></span>

<span data-ttu-id="fd7b1-424">İçinde **Azure App Service**, kaynak denetiminden sürekli dağıtım yapar, bu olası teşekkür **yeniden** eylemi Yönetim Portalı'nda kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-424">In **Azure App Service**, continuous deployment from source control makes this possible thanks to the **redeploy** action available in the management portal.</span></span> <span data-ttu-id="fd7b1-425">Azure, depoya itilmiş yürütmeleri ile ilişkilendirilen dağıtımları izler ve herhangi bir zamanda önceki dağıtımlarınızı birini kullanarak uygulamanızı yeniden dağıtmaya yönelik bir seçenek sağlar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-425">Azure keeps track of the deployments associated with the commits pushed to the repository and provides an option to redeploy your application using any of your previous deployments, at any time.</span></span>

<span data-ttu-id="fd7b1-426">Bu alıştırmada, kodda değişiklik gerçekleştirecek **Geek test** kasıtlı olarak eklediği uygulama bir *hata*.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-426">In this exercise you will perform a change to the code in the **Geek Quiz** application that intentionally injects a *bug*.</span></span> <span data-ttu-id="fd7b1-427">Hatayı görmek için uygulamayı üretime dağıtır ve ardından önceki duruma geri dönmek için yeniden dağıtma özelliğin avantajlarından yararlanmak.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-427">You will deploy the application to production to see the error, and then you will take advantage of the redeploy feature to go back to the previous state.</span></span>

<a id="Ex3Task1"></a>
#### <a name="task-1--updating-the-geek-quiz-application"></a><span data-ttu-id="fd7b1-428">Görev 1 – Geek test uygulamayı güncelleştirme</span><span class="sxs-lookup"><span data-stu-id="fd7b1-428">Task 1 – Updating the Geek Quiz Application</span></span>

<span data-ttu-id="fd7b1-429">Bu görevde, küçük bir parça kodu yeniden düzenleyin **TriviaController** seçili test seçeneği, yeni bir yönteme veritabanından alır mantığının parçası ayıklamak için sınıf.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-429">In this task, you will refactor a small piece of code of the **TriviaController** class to extract part of the logic that retrieves the selected quiz option from the database into a new method.</span></span>

1. <span data-ttu-id="fd7b1-430">Geçiş ile Visual Studio örneğine **GeekQuiz** önceki alıştırmada çözümünden.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-430">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="fd7b1-431">İçinde **Çözüm Gezgini**açın **TriviaController.cs** içinde dosya **denetleyicileri** klasör.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-431">In **Solution Explorer**, open the **TriviaController.cs** file inside the **Controllers** folder.</span></span>
3. <span data-ttu-id="fd7b1-432">Bulun **StoreAsync** yöntemi ve kodu vurgulanmış aşağıdaki şekilde seçin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-432">Locate the **StoreAsync** method and select the code highlighted in the following figure.</span></span>

    ![Kod seçme](maintainable-azure-websites-managing-change-and-scale/_static/image47.png)

    <span data-ttu-id="fd7b1-434">*Kod seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-434">*Selecting the code*</span></span>
4. <span data-ttu-id="fd7b1-435">Seçilen koda sağ tıklayın, genişletin **yeniden düzenleyin** menü ve select **yöntemi ayıkla...** .</span><span class="sxs-lookup"><span data-stu-id="fd7b1-435">Right-click the selected code, expand the **Refactor** menu and select **Extract Method...**.</span></span>

    ![Yeni bir yöntem kod ayıklanıyor](maintainable-azure-websites-managing-change-and-scale/_static/image48.png)

    <span data-ttu-id="fd7b1-437">*Ayıklama yöntemi seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-437">*Selecting Extract Method*</span></span>
5. <span data-ttu-id="fd7b1-438">İçinde **yöntemi ayıklama** iletişim kutusunda, yeni yöntemin adı *MatchesOption* tıklatıp **Tamam**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-438">In the **Extract Method** dialog box, name the new method *MatchesOption* and click **OK**.</span></span>

    ![Yöntem adı belirtme](maintainable-azure-websites-managing-change-and-scale/_static/image49.png)

    <span data-ttu-id="fd7b1-440">*Ayıklanan Metoda için ad belirtme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-440">*Specifying the name for the extracted method*</span></span>
6. <span data-ttu-id="fd7b1-441">Seçilen kod içine ayıklanır **MatchesOption** yöntemi.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-441">The selected code is then extracted into the **MatchesOption** method.</span></span> <span data-ttu-id="fd7b1-442">Sonuç kodu aşağıdaki kod parçacığında gösterilir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-442">The resulting code is shown in the following snippet.</span></span>

    [!code-csharp[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample14.cs)]
7. <span data-ttu-id="fd7b1-443">Tuşuna **CTRL + S** değişiklikleri kaydedin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-443">Press **CTRL + S** to save the changes.</span></span>

<a id="Ex3Task2"></a>
#### <a name="task-2--redeploying-the-geek-quiz-application"></a><span data-ttu-id="fd7b1-444">Görev 2 – Geek sınavı uygulamanızı</span><span class="sxs-lookup"><span data-stu-id="fd7b1-444">Task 2 – Redeploying the Geek Quiz Application</span></span>

<span data-ttu-id="fd7b1-445">Artık üretim ortamına yeni bir dağıtımı tetikleyecek depo önceki görevde yapılan değişiklikleri gönderir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-445">You will now push the changes you made in the previous task to the repository, which will trigger a new deployment to the production environment.</span></span> <span data-ttu-id="fd7b1-446">Ardından, troubleshot kullanarak bir sorun **F12 geliştirme araçları** Internet Explorer tarafından sağlanan ve ardından önceki dağıtım için bir geri alma Azure Yönetim Portalı'ndan gerçekleştirin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-446">Then, you will troubleshot an issue using the **F12 development tools** provided by Internet Explorer, and then perform a rollback to the previous deployment from the Azure management portal.</span></span>

1. <span data-ttu-id="fd7b1-447">Yeni bir **Git Bash** güncelleştirilmiş uygulamayı Azure App Service'e dağıtmak için konsolu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-447">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
2. <span data-ttu-id="fd7b1-448">Değişiklikleri Azure'a göndermek için aşağıdaki komutları yürütün.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-448">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="fd7b1-449">Güncelleştirme *[YOUR-uygulaması-PATH]* yolu ile yer tutucu **GeekQuiz** çözüm.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-449">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="fd7b1-450">Dağıtım parolanızı istenir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-450">You will be prompted for your deployment password.</span></span>
    ...
    <span data-ttu-id="fd7b1-452">*İşlenmiş kod Azure'a gönderme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-452">*Pushing refactored code to Azure*</span></span>
3. <span data-ttu-id="fd7b1-453">Internet Explorer'ı açın ve web uygulamanıza gidin (örn `http://<your-web-site>.azurewebsites.net`).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-453">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="fd7b1-454">Önceden oluşturulmuş kimlik bilgilerini kullanarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-454">Log in using the previously created credentials.</span></span>
4. <span data-ttu-id="fd7b1-455">Tuşuna **F12** Geliştirme Araçları'nı başlatmak için **ağ** sekmesine **yürütmek** düğmesi kaydı başlatın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-455">Press **F12** to launch the development tools, select the **Network** tab and click the **Play** button to start recording.</span></span>

    <span data-ttu-id="fd7b1-456">![Ağ kayıt başlangıç](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "ağ kayıt başlatılıyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-456">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image51.png "Starting network recording")</span></span>

    <span data-ttu-id="fd7b1-457">*Ağ kayıt başlatılıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-457">*Starting network recording*</span></span>
5. <span data-ttu-id="fd7b1-458">Test herhangi bir seçenek seçin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-458">Select any option of the quiz.</span></span> <span data-ttu-id="fd7b1-459">Hiçbir şey olmuyor görürsünüz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-459">You will see that nothing happens.</span></span>
6. <span data-ttu-id="fd7b1-460">İçinde **F12** penceresini bir HTTP POST HTTP isteğine karşılık gelen girişi göstermektedir **500** sonucu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-460">In the **F12** window, the entry corresponding to the POST HTTP request shows an HTTP **500** result.</span></span>

    ![HTTP 500 hata](maintainable-azure-websites-managing-change-and-scale/_static/image52.png)

    <span data-ttu-id="fd7b1-462">*HTTP 500 hata*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-462">*HTTP 500 error*</span></span>
7. <span data-ttu-id="fd7b1-463">Seçin **konsol** sekmesi. Bir hata nedeninin ayrıntılarını günlüğe kaydedilir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-463">Select the **Console** tab. An error is logged with the details of the cause.</span></span>

    ![Oturum hata](maintainable-azure-websites-managing-change-and-scale/_static/image53.png)

    <span data-ttu-id="fd7b1-465">*Oturum hata*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-465">*Logged error*</span></span>
8. <span data-ttu-id="fd7b1-466">Hata ayrıntıları bölümü bulun.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-466">Locate the details part of the error.</span></span> <span data-ttu-id="fd7b1-467">NET bir şekilde, önceki adımda kaydedilen yeniden düzenleme kod tarafından bu hataya neden.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-467">Clearly, this error is caused by the code refactoring you committed in the previous steps.</span></span>

    <span data-ttu-id="fd7b1-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-468">`Details: LINQ to Entities does not recognize the method 'Boolean MatchesOption ...`.</span></span>
9. <span data-ttu-id="fd7b1-469">Tarayıcı kapatmayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-469">Do not close the browser.</span></span>
10. <span data-ttu-id="fd7b1-470">Yeni bir tarayıcı örneğinde gidin [Azure Yönetim Portalı](https://manage.windowsazure.com) aboneliğinizle ilişkili Microsoft hesabını kullanarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-470">In a new browser instance, navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
11. <span data-ttu-id="fd7b1-471">Seçin **Web siteleri** ve alıştırma 2'de oluşturduğunuz web uygulamasına tıklayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-471">Select **Websites** and click the web app you created in Exercise 2.</span></span>
12. <span data-ttu-id="fd7b1-472">Gidin **dağıtımları** sayfası.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-472">Navigate to the **Deployments** page.</span></span> <span data-ttu-id="fd7b1-473">Dağıtım geçmişi gerçekleştirilen tüm işlemeleri listelendiğine dikkat edin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-473">Notice that all the commits performed are listed in the deployment history.</span></span>

    ![Var olan dağıtımları listesi](maintainable-azure-websites-managing-change-and-scale/_static/image54.png)

    <span data-ttu-id="fd7b1-475">*Var olan dağıtımları listesi*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-475">*List of existing deployments*</span></span>
13. <span data-ttu-id="fd7b1-476">Önceki işlemeyi seçip tıklayın **yeniden** komut çubuğunda.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-476">Select the previous commit and click **Redeploy** on the command bar.</span></span>

    ![Önceki işlemeyi yeniden dağıtılıyor](maintainable-azure-websites-managing-change-and-scale/_static/image55.png)

    <span data-ttu-id="fd7b1-478">*Önceki işlemeyi yeniden dağıtılıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-478">*Redeploying the previous commit*</span></span>
14. <span data-ttu-id="fd7b1-479">Onaylamak için sorulduğunda **Evet**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-479">When prompted to confirm, click **Yes**.</span></span>

    ![Onaylayan yeniden dağıtma](maintainable-azure-websites-managing-change-and-scale/_static/image56.png)
15. <span data-ttu-id="fd7b1-481">Dağıtım tamamlandığında, web uygulaması ve tuşuna tarayıcı örneğiyle dönmek **CTRL + F5 tuşlarına basarak**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-481">When the deployment completes, switch back to the browser instance with your web app and press **CTRL + F5**.</span></span>
16. <span data-ttu-id="fd7b1-482">Seçeneklerden birini tıklatın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-482">Click any of the options.</span></span> <span data-ttu-id="fd7b1-483">Çevirme animasyon yerinde ve sonucu şimdi al (*düzeltmek/yanlış*) görüntülenir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-483">The flip animation will now take place and the result (*correct/incorrect*) will be displayed.</span></span>
17. <span data-ttu-id="fd7b1-484">(İsteğe bağlı) Geçiş **Git Bash** konsol ve için önceki yürütmeyi dönmek için aşağıdaki komutları yürütün.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-484">(Optional) Switch to the **Git Bash** console and execute the following commands to revert to the previous commit.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-485">Bu komutlar Git deposundaki hatalı işlemede yapılan tüm değişiklikleri geri alır, yeni bir işleme oluşturur.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-485">These commands create a new commit that undoes all changes in the Git repository that were made in the bad commit.</span></span> <span data-ttu-id="fd7b1-486">Azure, ardından yeni işleme kullanarak uygulamayı yeniden dağıtır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-486">Azure will then redeploy the application using the new commit.</span></span>

    [!code-console[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample16.cmd)]

<a id="Exercise4"></a>
### <a name="exercise-4-scaling-using-azure-storage"></a><span data-ttu-id="fd7b1-487">Alıştırma 4: Azure depolama kullanarak ölçeklendirme</span><span class="sxs-lookup"><span data-stu-id="fd7b1-487">Exercise 4: Scaling Using Azure Storage</span></span>

<span data-ttu-id="fd7b1-488">**Blobları** büyük miktarda yapılandırılmamış metin ve video, ses ve görüntü gibi ikili verileri depolamanın en kolay yoludur.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-488">**Blobs** are the simplest way to store large amounts of unstructured text or binary data such as video, audio and images.</span></span> <span data-ttu-id="fd7b1-489">Depolama uygulamanıza statik içeriği taşıma, görüntülerin veya belgelerin doğrudan tarayıcı sunarak uygulamanızın ölçeğini yardımcı olur.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-489">Moving the static content of your application to Storage, helps to scale your application by serving images or documents directly to the browser.</span></span>

<span data-ttu-id="fd7b1-490">Bu alıştırmada, statik içerik, uygulamanızın bir Blob kapsayıcısını taşınır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-490">In this exercise, you will move the static content of your application to a Blob container.</span></span> <span data-ttu-id="fd7b1-491">Uygulamanızı eklemek için yapılandıracağınız sonra bir **ASP.NET URL yeniden yazma kuralı** içinde **Web.config** içeriğinizi Blob kapsayıcısına yönlendirmek için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-491">Then you will configure your application to add an **ASP.NET URL rewrite rule** in the **Web.config** to redirect your content to the Blob container.</span></span>

<a id="Ex4Task1"></a>
#### <a name="task-1--creating-an-azure-storage-account"></a><span data-ttu-id="fd7b1-492">Görev 1-bir Azure depolama hesabı oluşturma</span><span class="sxs-lookup"><span data-stu-id="fd7b1-492">Task 1 – Creating an Azure Storage Account</span></span>

<span data-ttu-id="fd7b1-493">Bu görevde, yönetim portalını kullanarak yeni bir depolama hesabının nasıl oluşturulacağını öğreneceksiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-493">In this task you will learn how to create a new storage account using the management portal.</span></span>

1. <span data-ttu-id="fd7b1-494">Gidin [Azure Yönetim Portalı](https://manage.windowsazure.com) aboneliğinizle ilişkili Microsoft hesabını kullanarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-494">Navigate to the [Azure management portal](https://manage.windowsazure.com) and sign in using the Microsoft account associated with your subscription.</span></span>
2. <span data-ttu-id="fd7b1-495">Seçin **yeni | Veri Hizmetleri | Depolama | Hızlı Oluştur** yeni bir depolama hesabı oluşturmaya başlamak için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-495">Select **New | Data Services | Storage | Quick Create** to start creating a new storage account.</span></span> <span data-ttu-id="fd7b1-496">Seçin ve hesabı için benzersiz bir ad girin. bir **bölge** listeden.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-496">Enter a unique name for the account and select a **Region** from the list.</span></span> <span data-ttu-id="fd7b1-497">Tıklayın **depolama hesabı oluştur** devam etmek için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-497">Click **Create Storage Account** to continue.</span></span>

    <span data-ttu-id="fd7b1-498">![Yeni bir depolama hesabı oluşturma](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "yeni bir depolama hesabı oluşturma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-498">![Creating a new Storage Account](maintainable-azure-websites-managing-change-and-scale/_static/image57.png "Creating a new Storage Account")</span></span>

    <span data-ttu-id="fd7b1-499">*Yeni bir depolama hesabı oluşturma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-499">*Creating a new storage account*</span></span>
3. <span data-ttu-id="fd7b1-500">İçinde **depolama** bölümünde, yeni depolama hesabı durumunu olana kadar bekleyin *çevrimiçi* aşağıdaki adım ile devam etmek için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-500">In the **Storage** section, wait until the status of the new storage account changes to *Online* in order to continue with the following step.</span></span>

    <span data-ttu-id="fd7b1-501">![Oluşturduğunuz depolama hesabına](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "depolama hesabı oluşturuldu")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-501">![Storage Account created](maintainable-azure-websites-managing-change-and-scale/_static/image58.png "Storage Account created")</span></span>

    <span data-ttu-id="fd7b1-502">*Depolama hesabı oluşturuldu*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-502">*Storage Account created*</span></span>
4. <span data-ttu-id="fd7b1-503">Depolama hesabının adına tıklayın ve ardından **Pano** sayfanın üstündeki bağlantısı.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-503">Click on the storage account name and then click the **Dashboard** link at the top of the page.</span></span> <span data-ttu-id="fd7b1-504">**Pano** sayfası durumuyla ilgili bilgileri hesabınızın ve uygulamalarınızın içinde kullanılabilir hizmet uç noktaları ile sağlar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-504">The **Dashboard** page provides you with information about the status of the account and the service endpoints that can be used within your applications.</span></span>

    <span data-ttu-id="fd7b1-505">![Depolama hesabı panoyu görüntüleme](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "depolama hesabı panoyu görüntüleme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-505">![Displaying the Storage Account Dashboard](maintainable-azure-websites-managing-change-and-scale/_static/image59.png "Displaying the Storage Account Dashboard")</span></span>

    <span data-ttu-id="fd7b1-506">*Depolama hesabı panoyu görüntüleme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-506">*Displaying the Storage Account Dashboard*</span></span>
5. <span data-ttu-id="fd7b1-507">Tıklayın **erişim anahtarlarını Yönet** gezinti çubuğunda düğme.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-507">Click the **Manage Access Keys** button in the navigation bar.</span></span>

    <span data-ttu-id="fd7b1-508">![Yönet erişim anahtarları düğmesi](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "erişim anahtarlarını Yönet düğmesi")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-508">![Manage Access Keys button](maintainable-azure-websites-managing-change-and-scale/_static/image60.png "Manage Access Keys button")</span></span>

    <span data-ttu-id="fd7b1-509">*Erişim anahtarları düğmesi yönetme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-509">*Manage Access Keys button*</span></span>
6. <span data-ttu-id="fd7b1-510">İçinde **erişim anahtarlarını Yönet** iletişim kutusu, kopyalama **depolama hesabı adı** ve **birincil erişim anahtarı** alıştırmada ihtiyaç duyacaksınız.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-510">In the **Manage Access Keys** dialog box, copy the **Storage Account Name** and **Primary Access Key** as you will need them in the following exercise.</span></span> <span data-ttu-id="fd7b1-511">Ardından, iletişim kutusunu kapatın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-511">Then, close the dialog box.</span></span>

    <span data-ttu-id="fd7b1-512">![Erişim anahtarı iletişim kutusu Yönet](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "erişim anahtarını yönetin iletişim kutusu")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-512">![Manage Access Key dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image61.png "Manage Access Key dialog box")</span></span>

    <span data-ttu-id="fd7b1-513">*Erişim anahtarı iletişim kutusu yönetme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-513">*Manage Access Key dialog box*</span></span>

<a id="Ex4Task2"></a>
#### <a name="task-2--uploading-an-asset-to-azure-blob-storage"></a><span data-ttu-id="fd7b1-514">Görev 2-Azure Blob depolama alanına bir varlık karşıya yükleniyor</span><span class="sxs-lookup"><span data-stu-id="fd7b1-514">Task 2 – Uploading an Asset to Azure Blob Storage</span></span>

<span data-ttu-id="fd7b1-515">Bu görevde, depolama hesabınıza bağlanmak için Visual Studio Sunucu Gezgini penceresinde kullanır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-515">In this task, you will use the Server Explorer window from Visual Studio to connect to your storage account.</span></span> <span data-ttu-id="fd7b1-516">Ardından, bir blob kapsayıcı oluşturun ve kapsayıcıya Geek test logosu bir dosyayı karşıya.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-516">You will then create a blob container and upload a file with the Geek Quiz logo to the container.</span></span>

1. <span data-ttu-id="fd7b1-517">Geçiş ile Visual Studio örneğine **GeekQuiz** önceki alıştırmada çözümünden.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-517">Switch to the Visual Studio instance with the **GeekQuiz** solution from the previous exercise.</span></span>
2. <span data-ttu-id="fd7b1-518">Menü çubuğundan seçin **görünümü** ve ardından **Sunucu Gezgini**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-518">From the menu bar, select **View** and then click **Server Explorer**.</span></span>
3. <span data-ttu-id="fd7b1-519">İçinde **Sunucu Gezgini**, sağ **Azure** düğümünü seçip alt **azure'a Bağlan...** . Aboneliğinizle ilişkili Microsoft hesabını kullanarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-519">In **Server Explorer**, right-click the **Azure** node and select **Connect to Azure...**. Sign in using the Microsoft account associated with your subscription.</span></span>

    ![Windows Azure'a bağlanma](maintainable-azure-websites-managing-change-and-scale/_static/image62.png)

    <span data-ttu-id="fd7b1-521">*Azure'a bağlanma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-521">*Connect to Azure*</span></span>
4. <span data-ttu-id="fd7b1-522">Genişletin **Azure** düğümünü sağ **depolama** seçip **dış depolama Ekle...** .</span><span class="sxs-lookup"><span data-stu-id="fd7b1-522">Expand the **Azure** node, right-click **Storage** and select **Attach External Storage...**.</span></span>
5. <span data-ttu-id="fd7b1-523">İçinde **yeni depolama hesabı Ekle** iletişim kutusuna **hesap adı** ve **hesap anahtarı** tıklayın ve önceki görev içinde elde edilen **Tamam**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-523">In the **Add New Storage Account** dialog box, enter the **Account name** and **Account key** you obtained in the previous task and click **OK**.</span></span>

    ![Yeni depolama hesabı iletişim kutusu Ekle](maintainable-azure-websites-managing-change-and-scale/_static/image63.png)

    <span data-ttu-id="fd7b1-525">*Yeni depolama hesabı iletişim kutusu Ekle*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-525">*Add New Storage Account dialog box*</span></span>
6. <span data-ttu-id="fd7b1-526">Depolama hesabınızın altında görünmelidir **depolama** düğümü.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-526">Your storage account should appear under the **Storage** node.</span></span> <span data-ttu-id="fd7b1-527">Depolama hesabınızı genişletin, sağ **Blobları** seçip **Blob kapsayıcısı oluştur...** .</span><span class="sxs-lookup"><span data-stu-id="fd7b1-527">Expand your storage account, right-click **Blobs** and select **Create Blob Container...**.</span></span>

    <span data-ttu-id="fd7b1-528">![BLOB kapsayıcısı oluşturma](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Blob kapsayıcısı oluşturma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-528">![Create Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image64.png "Create Blob Container")</span></span>

    <span data-ttu-id="fd7b1-529">*BLOB kapsayıcısı oluşturma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-529">*Create Blob Container*</span></span>
7. <span data-ttu-id="fd7b1-530">İçinde **Blob kapsayıcısı Oluştur** iletişim kutusuna blob kapsayıcısı için bir ad girin ve tıklatın **Tamam**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-530">In the **Create Blob Container** dialog box, enter a name for the blob container and click **OK**.</span></span>

    <span data-ttu-id="fd7b1-531">![Blob kapsayıcısı iletişim kutusu oluşturma](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Blob kapsayıcısı Oluştur iletişim kutusu")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-531">![Create Blob Container dialog box](maintainable-azure-websites-managing-change-and-scale/_static/image65.png "Create Blob Container dialog box")</span></span>

    <span data-ttu-id="fd7b1-532">*BLOB kapsayıcısı iletişim kutusu oluşturma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-532">*Create Blob Container dialog box*</span></span>
8. <span data-ttu-id="fd7b1-533">Yeni blob kapsayıcı eklenmelidir **Blobları** düğümü.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-533">The new blob container should be added to the **Blobs** node.</span></span> <span data-ttu-id="fd7b1-534">Kapsayıcı genel hale getirmek üzere kapsayıcıda erişim izinlerini değiştirin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-534">Change the access permissions in the container to make the container public.</span></span> <span data-ttu-id="fd7b1-535">Bunu yapmak için sağ **görüntüleri** kapsayıcı ve select **özellikleri**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-535">To do this, right-click the **images** container and select **Properties**.</span></span>

    <span data-ttu-id="fd7b1-536">![kapsayıcı özellikleri görüntüleri](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "görüntüler kapsayıcı özellikleri")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-536">![images container properties](maintainable-azure-websites-managing-change-and-scale/_static/image66.png "images container properties")</span></span>

    <span data-ttu-id="fd7b1-537">*Görüntüler kapsayıcı özellikleri*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-537">*Images container properties*</span></span>
9. <span data-ttu-id="fd7b1-538">İçinde **özellikleri** penceresinde **genel okuma erişimini** için **kapsayıcı**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-538">In the **Properties** window, set the **Public Read Access** to **Container**.</span></span>

    <span data-ttu-id="fd7b1-539">![Genel okuma erişimi özelliği değiştirme](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "genel okuma erişimi özelliği değiştirme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-539">![Changing public read access property](maintainable-azure-websites-managing-change-and-scale/_static/image67.png "Changing public read access property")</span></span>

    <span data-ttu-id="fd7b1-540">*Genel okuma erişimi özelliği değiştirme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-540">*Changing public read access property*</span></span>
10. <span data-ttu-id="fd7b1-541">Emin olup olmadığınız sorulduğunda, istediğiniz genel erişim özelliği değiştirmek **Evet**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-541">When prompted if you are sure you want to change the public access property, click **Yes**.</span></span>

    <span data-ttu-id="fd7b1-542">![Microsoft Visual Studio uyarı](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio Uyarısı")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-542">![Microsoft Visual Studio warning](maintainable-azure-websites-managing-change-and-scale/_static/image68.png "Microsoft Visual Studio warning")</span></span>

    <span data-ttu-id="fd7b1-543">*Microsoft Visual Studio Uyarısı*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-543">*Microsoft Visual Studio warning*</span></span>
11. <span data-ttu-id="fd7b1-544">İçinde **Sunucu Gezgini**, sağ **görüntüleri** blob kapsayıcısını ve select **görünüm Blob kapsayıcısını**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-544">In **Server Explorer**, right-click in the **images** blob container and select **View Blob Container**.</span></span>

    <span data-ttu-id="fd7b1-545">![Blob kapsayıcısı görüntülemek](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "Blob kapsayıcısı görüntüleyin")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-545">![View Blob Container](maintainable-azure-websites-managing-change-and-scale/_static/image69.png "View Blob Container")</span></span>

    <span data-ttu-id="fd7b1-546">*Görünüm Blob kapsayıcısı*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-546">*View Blob Container*</span></span>
12. <span data-ttu-id="fd7b1-547">Görüntü kapsayıcısı yeni bir pencerede açılması gerekir; hiçbir girdi bir gösterge gösterilecek.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-547">The images container should open in a new window and a legend with no entries should be shown.</span></span> <span data-ttu-id="fd7b1-548">Tıklayın **karşıya** simgesini bir dosyayı blob kapsayıcısına yükleyin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-548">Click the **upload** icon to upload a file to the blob container.</span></span>

    <span data-ttu-id="fd7b1-549">![Hiçbir girdi ile kapsayıcı görüntüleri](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "hiçbir girdi ile kapsayıcı görüntüleri")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-549">![Images container with no entries](maintainable-azure-websites-managing-change-and-scale/_static/image70.png "Images container with no entries")</span></span>

    <span data-ttu-id="fd7b1-550">*Görüntü kapsayıcısı ile giriş yok*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-550">*Images container with no entries*</span></span>
13. <span data-ttu-id="fd7b1-551">İçinde **Blob karşıya** iletişim kutusunda, gitmek **varlıklar** laboratuvarı klasör.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-551">In the **Upload Blob** dialog box, navigate to the **Assets** folder of the lab.</span></span> <span data-ttu-id="fd7b1-552">Seçin **logosu big.png** tıklayın ve dosya **açık**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-552">Select the **logo-big.png** file and click **Open**.</span></span>
14. <span data-ttu-id="fd7b1-553">Dosya karşıya kadar bekleyin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-553">Wait until the file is uploaded.</span></span> <span data-ttu-id="fd7b1-554">Karşıya yükleme tamamlandığında, dosya görüntüleri kapsayıcısında listelenmelidir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-554">When the upload completes, the file should be listed in the images container.</span></span> <span data-ttu-id="fd7b1-555">Dosya giriş sağ tıklayıp **kopya URL**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-555">Right-click the file entry and select **Copy URL**.</span></span>

    <span data-ttu-id="fd7b1-556">![BLOB URL'sini kopyalayın](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "kopyalama blob dosya URL'si")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-556">![Copy blob URL](maintainable-azure-websites-managing-change-and-scale/_static/image71.png "Copy blob file URL")</span></span>

    <span data-ttu-id="fd7b1-557">*BLOB URL'sini Kopyala*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-557">*Copy blob URL*</span></span>
15. <span data-ttu-id="fd7b1-558">Internet Explorer'ı açın ve URL'yi yapıştırın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-558">Open Internet Explorer and paste the URL.</span></span> <span data-ttu-id="fd7b1-559">Aşağıdaki görüntüde, tarayıcı içinde gösterilmesi gerekir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-559">The following image should be shown in the browser.</span></span>

    <span data-ttu-id="fd7b1-560">![Windows Blob depolamadan big.png logo görüntüsü](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logosu big.png görüntüden depolama")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-560">![logo-big.png image from Windows Blob Storage](maintainable-azure-websites-managing-change-and-scale/_static/image72.png "logo-big.png image from storage")</span></span>

    <span data-ttu-id="fd7b1-561">*Azure Blob Depolama'dan big.png logo görüntüsü*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-561">*logo-big.png image from Azure Blob Storage*</span></span>

<a id="Ex4Task3"></a>
#### <a name="task-3--updating-the-solution-to-consume-static-content-from-azure-blob-storage"></a><span data-ttu-id="fd7b1-562">Görev 3: Azure Blob depolamadan statik içeriği çözüm güncelleştiriliyor</span><span class="sxs-lookup"><span data-stu-id="fd7b1-562">Task 3 – Updating the Solution to Consume Static Content from Azure Blob Storage</span></span>

<span data-ttu-id="fd7b1-563">Bu görevde, yapılandıracağınız **GeekQuiz** görüntüyü kullanmak için çözüm karşıya Azure Blob depolama alanına (yerine web uygulamasında yer alan görüntü) bir ASP.NET URL yeniden yazma kuralı ekleyerek **web.config**dosya.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-563">In this task, you will configure the **GeekQuiz** solution to consume the image uploaded to Azure Blob Storage (instead of the image located in the web app) by adding an ASP.NET URL rewrite rule in the **web.config** file.</span></span>

1. <span data-ttu-id="fd7b1-564">Visual Studio'da açın **Web.config** içinde dosya **GeekQuiz** bulun ve proje **&lt;system.webServer&gt;** öğesi.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-564">In Visual Studio, open the **Web.config** file inside the **GeekQuiz** project and locate the **&lt;system.webServer&gt;** element.</span></span>
2. <span data-ttu-id="fd7b1-565">Bir URL, yer tutucu ile depolama hesabınızın adını güncelleştirme kuralı, yeniden yazma eklemek için aşağıdaki kodu ekleyin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-565">Add the following code to add an URL rewrite rule, updating the placeholder with your storage account name.</span></span>

    <span data-ttu-id="fd7b1-566">(Kod parçacığını - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span><span class="sxs-lookup"><span data-stu-id="fd7b1-566">(Code Snippet - *WebSitesInProduction - Ex4 - UrlRewriteRule*)</span></span>

    [!code-xml[Main](maintainable-azure-websites-managing-change-and-scale/samples/sample17.xml)]

    > [!NOTE]
    > <span data-ttu-id="fd7b1-567">URL yeniden yazma, gelen bir Web isteği kesintiye ve istek farklı bir kaynak için yönlendirme işlemidir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-567">URL rewriting is the process of intercepting an incoming Web request and redirecting the request to a different resource.</span></span> <span data-ttu-id="fd7b1-568">URL yeniden yazma kuralları, bir istek yönlendirilmesi gerektiğinde ve burada yönlendirilen yeniden yazma motoru söyler.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-568">The URL rewriting rules tells the rewriting engine when a request needs to be redirected, and where should they be redirected.</span></span> <span data-ttu-id="fd7b1-569">Bir yeniden yazma kuralı iki dizeleri kümesinden oluşur: İstenen URL'de aranacak desenini (genellikle, normal ifadeler kullanarak) ve varsa, deseni değiştirilecek dize bulundu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-569">A rewriting rule is composed of two strings: the pattern to look for in the requested URL (usually, using regular expressions), and the string to replace the pattern with, if found.</span></span> <span data-ttu-id="fd7b1-570">Daha fazla bilgi için [URL yeniden yazma ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-570">For more information, see [URL Rewriting in ASP.NET](https://msdn.microsoft.com/library/ms972974.aspx).</span></span>
3. <span data-ttu-id="fd7b1-571">Tuşuna **CTRL + S** değişiklikleri kaydedin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-571">Press **CTRL + S** to save the changes.</span></span>
4. <span data-ttu-id="fd7b1-572">Yeni bir **Git Bash** güncelleştirilmiş uygulamayı Azure App Service'e dağıtmak için konsolu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-572">Open a new **Git Bash** console to deploy the updated application to Azure App Service.</span></span>
5. <span data-ttu-id="fd7b1-573">Değişiklikleri Azure'a göndermek için aşağıdaki komutları yürütün.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-573">Execute the following commands to push the changes to Azure.</span></span> <span data-ttu-id="fd7b1-574">Güncelleştirme *[YOUR-uygulaması-PATH]* yolu ile yer tutucu **GeekQuiz** çözüm.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-574">Update the *[YOUR-APPLICATION-PATH]* placeholder with the path to the **GeekQuiz** solution.</span></span> <span data-ttu-id="fd7b1-575">Dağıtım parolanızı istenir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-575">You will be prompted for your deployment password.</span></span>
    ...
    <span data-ttu-id="fd7b1-577">*Güncelleştirme Azure'a dağıtma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-577">*Deploying update to Azure*</span></span>

<a id="Ex4Task4"></a>
#### <a name="task-4--verification"></a><span data-ttu-id="fd7b1-578">Görev 4 – doğrulama</span><span class="sxs-lookup"><span data-stu-id="fd7b1-578">Task 4 – Verification</span></span>

<span data-ttu-id="fd7b1-579">Bu görevde kullanacağınız **Internet Explorer** göz atmak için **Geek test** görüntüye barındırılan uygulama ve URL yeniden yazma, görüntüleri çalışır ve kuralı onay yönlendirilir **Azure Blob Depolama**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-579">In this task you will use **Internet Explorer** to browse the **Geek Quiz** application and check that the URL rewrite rule for images works and you are redirected to the image hosted on **Azure Blob Storage**.</span></span>

1. <span data-ttu-id="fd7b1-580">Internet Explorer'ı açın ve web uygulamanıza gidin (örn `http://<your-web-site>.azurewebsites.net`).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-580">Open Internet Explorer and navigate to your web app (e.g. `http://<your-web-site>.azurewebsites.net`).</span></span> <span data-ttu-id="fd7b1-581">Önceden oluşturulmuş kimlik bilgilerini kullanarak oturum açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-581">Log in using the previously created credentials.</span></span>

    <span data-ttu-id="fd7b1-582">![Görüntüyle Geek test web uygulamasını gösteren](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "görüntüyle Geek test web uygulamasını gösteren")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-582">![Showing the Geek Quiz web app with the image](maintainable-azure-websites-managing-change-and-scale/_static/image74.png "Showing the Geek Quiz web app with the image")</span></span>

    <span data-ttu-id="fd7b1-583">*Geek test web uygulaması ile görüntü gösterme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-583">*Showing the Geek Quiz web app with the image*</span></span>
2. <span data-ttu-id="fd7b1-584">Tuşuna **F12** Geliştirme Araçları'nı başlatmak için **ağ** sekme ve kaydı başlatın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-584">Press **F12** to launch the development tools, select the **Network** tab and start recording.</span></span>

    <span data-ttu-id="fd7b1-585">![Ağ kayıt başlangıç](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "ağ kayıt başlatılıyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-585">![Starting network recording](maintainable-azure-websites-managing-change-and-scale/_static/image75.png "Starting network recording")</span></span>

    <span data-ttu-id="fd7b1-586">*Ağ kayıt başlatılıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-586">*Starting network recording*</span></span>
3. <span data-ttu-id="fd7b1-587">Tuşuna **CTRL + F5 tuşlarına basarak** web sayfasını yenilemek için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-587">Press **CTRL + F5** to refresh the web page.</span></span>
4. <span data-ttu-id="fd7b1-588">Sayfa yükleme tamamlandıktan sonra bir HTTP isteği için görmelisiniz **/img/logo-big.png** URL bir HTTP ile **301** sonucu (yeniden yönlendirme) ve başka bir istek `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL ile bir HTTP **200** sonucu.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-588">Once the page has finished loading, you should see an HTTP request for the **/img/logo-big.png** URL with an HTTP **301** result (redirect) and another request for `http://[YOUR-STORAGE-ACCOUNT].blob.core.windows.net/images/logo-big.png` URL with a HTTP **200** result.</span></span>

    <span data-ttu-id="fd7b1-589">![URL yeniden yönlendirme doğrulama](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "yeniden yönlendirme gösteren geliştirme araçları")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-589">![Verifying the URL redirect](maintainable-azure-websites-managing-change-and-scale/_static/image76.png "Showing the redirect in Dev Tools")</span></span>

    <span data-ttu-id="fd7b1-590">*URL yeniden yönlendirme doğrulanıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-590">*Verifying the URL redirect*</span></span>

<a id="Exercise5"></a>
### <a name="exercise-5-using-autoscale-for-web-apps"></a><span data-ttu-id="fd7b1-591">Alıştırma 5: Web Apps için otomatik ölçeklendirme kullanma</span><span class="sxs-lookup"><span data-stu-id="fd7b1-591">Exercise 5: Using Autoscale for Web Apps</span></span>

> [!NOTE]
> <span data-ttu-id="fd7b1-592">Web yükü için destek gerektirdiğinden bu alıştırmada, isteğe bağlı &amp; yalnızca için kullanılabilir olan performans testi **Visual Studio 2013 Ultimate Edition**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-592">This exercise is optional, since it requires support for Web Load &amp; Performance Testing which is only available for **Visual Studio 2013 Ultimate Edition**.</span></span> <span data-ttu-id="fd7b1-593">Belirli Visual Studio 2013 özellikleri hakkında daha fazla bilgi için sürümleri Karşılaştır [burada](https://www.microsoft.com/visualstudio/eng/products/compare).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-593">For more information on specific Visual Studio 2013 features, compare versions [here](https://www.microsoft.com/visualstudio/eng/products/compare).</span></span>


<span data-ttu-id="fd7b1-594">**Azure App Service Web Apps** çalışan web uygulamaları için otomatik ölçeklendirme özelliği sağlar **Standart mod**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-594">**Azure App Service Web Apps** provides the Autoscale feature for web apps running in **Standard Mode**.</span></span> <span data-ttu-id="fd7b1-595">Otomatik ölçeklendirme, Azure web uygulamanızı yüke bağlı olarak örnek sayısını otomatik olarak ölçeklendirme sağlar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-595">Autoscale lets Azure automatically scale the instance count of your web app depending on the load.</span></span> <span data-ttu-id="fd7b1-596">Otomatik ölçeklendirme etkin olduğunda, Azure web uygulamanızın CPU her beş dakikada denetler ve o noktasında örneği ekler.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-596">When Autoscale is enabled, Azure checks the CPU of your web app once every five minutes and adds instances as needed at that point in time.</span></span> <span data-ttu-id="fd7b1-597">CPU kullanımı düşükse, Azure web uygulamanızın performansını değil düzeyinin düşürüldüğünü emin olmak için her iki saatte bir kez örneklerini kaldırır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-597">If the CPU usage is low, Azure will remove instances once every two hours to ensure that the performance of your web app is not degraded.</span></span>

<span data-ttu-id="fd7b1-598">Bu alıştırmada yapılandırmak için gereken adımlarda size geçer **otomatik ölçeklendirme** için özellik **Geek test** web uygulaması.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-598">In this exercise you will go through the steps required to configure the **Autoscale** feature for the **Geek Quiz** web app.</span></span> <span data-ttu-id="fd7b1-599">Bu özellik, bir örneği yükseltme tetiklemek için uygulama üzerinde yeterli CPU yükü oluşturmak için Visual Studio Yük testi çalıştırarak doğrular.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-599">You will verify this feature by running a Visual Studio load test to generate enough CPU load on the application to trigger an instance upgrade.</span></span>

<a id="Ex5Task1"></a>
#### <a name="task-1--configuring-autoscale-based-on-the-cpu-metric"></a><span data-ttu-id="fd7b1-600">Görev 1 – CPU ölçüme göre yapılandırma otomatik ölçeklendirme</span><span class="sxs-lookup"><span data-stu-id="fd7b1-600">Task 1 – Configuring Autoscale Based on the CPU Metric</span></span>

<span data-ttu-id="fd7b1-601">Bu görevde alıştırma 2'de oluşturduğunuz web uygulaması için otomatik ölçeklendirme özelliği etkinleştirmek için Azure Yönetim Portalı'nı kullanır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-601">In this task you will use the Azure management portal to enable the Autoscale feature for the web app you created in Exercise 2.</span></span>

1. <span data-ttu-id="fd7b1-602">İçinde [Azure Yönetim Portalı](https://manage.windowsazure.com/)seçin **Web siteleri** ve alıştırma 2'de oluşturduğunuz web uygulamasına tıklayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-602">In the [Azure management portal](https://manage.windowsazure.com/), select **Websites** and click the web app you created in Exercise 2.</span></span>
2. <span data-ttu-id="fd7b1-603">Gidin **ölçek** sayfası.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-603">Navigate to the **Scale** page.</span></span> <span data-ttu-id="fd7b1-604">Altında **kapasite** bölümünden **CPU** için **ölçek ölçümü tarafından** yapılandırma.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-604">Under the **capacity** section, select **CPU** for the **Scale by Metric** configuration.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-605">CPU tarafından ölçeklendirme, Azure CPU kullanımı değişirse uygulamanın kullandığı örnek sayısını dinamik olarak ayarlar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-605">When scaling by CPU, Azure dynamically adjusts the number of instances that the app uses if the CPU usage changes.</span></span>

    <span data-ttu-id="fd7b1-606">![Ölçek CPU tarafından seçerek](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "otomatik ölçeklendirmeye yönelik CPU ölçüm seçme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-606">![Selecting to scale by CPU](maintainable-azure-websites-managing-change-and-scale/_static/image77.png "Selecting the CPU metric for auto scaling")</span></span>

    <span data-ttu-id="fd7b1-607">*Ölçek CPU'ya göre seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-607">*Selecting to scale by CPU*</span></span>
3. <span data-ttu-id="fd7b1-608">Değişiklik **hedef CPU** yapılandırmasına **20**-**40** yüzdesi.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-608">Change the **Target CPU** configuration to **20**-**40** percent.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-609">Bu aralık, web uygulamanız için ortalama CPU kullanımını gösterir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-609">This range represents the average CPU usage for your web app.</span></span> <span data-ttu-id="fd7b1-610">Azure eklemek veya web uygulamanız bu aralıkta tutmak için örnekler kaldırın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-610">Azure will add or remove instances to keep your web app in this range.</span></span> <span data-ttu-id="fd7b1-611">Ölçeklendirme için kullanılan örneklerin minimum ve maksimum sayı belirtilen **örnek sayısı** yapılandırma.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-611">The minimum and maximum number of instances used for scaling is specified in the **Instance Count** configuration.</span></span> <span data-ttu-id="fd7b1-612">Azure, yukarıda veya bu sınırı aşan hiçbir zaman geçer.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-612">Azure will never go above or beyond that limit.</span></span>
    >
    > <span data-ttu-id="fd7b1-613">Varsayılan **hedef CPU** değerleri yalnızca bu Laboratuvarın amacı doğrultusunda değiştirildi.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-613">The default **Target CPU** values are modified just for the purposes of this lab.</span></span> <span data-ttu-id="fd7b1-614">CPU aralığı küçük değerleri ile yapılandırarak, uygulama üzerinde orta dereceli bir yük yerleştirildiğinde tetikleyici otomatik ölçeklendirme için büyük olasılıkla artmaktadır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-614">By configuring the CPU range with small values, you are increasing the chances to trigger Autoscale when a moderate load is placed on the application.</span></span>

    <span data-ttu-id="fd7b1-615">![Hedef CPU 20 ve yüzde 40'a arasında olacak şekilde değiştirmeyi](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "20 ve yüzde 40'arasında olması için hedef CPU değiştirme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-615">![Changing the target CPU to be between 20 and 40 percent](maintainable-azure-websites-managing-change-and-scale/_static/image78.png "Changing the target CPU to be between 20 and 40 percent")</span></span>

    <span data-ttu-id="fd7b1-616">*Hedef CPU 20 ve yüzde 40'a arasında olacak şekilde değiştirme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-616">*Changing the Target CPU to be between 20 and 40 percent*</span></span>
4. <span data-ttu-id="fd7b1-617">Tıklayın **Kaydet** değişiklikleri kaydetmek için komut çubuğunda.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-617">Click **Save** in the command bar to save the changes.</span></span>

<a id="Ex5Task2"></a>
#### <a name="task-2--load-testing-with-visual-studio"></a><span data-ttu-id="fd7b1-618">Görev 2: yük testi Visual Studio ile</span><span class="sxs-lookup"><span data-stu-id="fd7b1-618">Task 2 – Load Testing with Visual Studio</span></span>

<span data-ttu-id="fd7b1-619">Şimdi **otomatik ölçeklendirme** olmuştur oluşturacağınız yapılandırıldı, bir **Web performansı ve yük testi projesi** biraz CPU yükü web uygulamanızı oluşturmak için Visual Studio'da.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-619">Now that **Autoscale** has been configured, you will create a **Web Performance and Load Test Project** in Visual Studio to generate some CPU load on your web app.</span></span>

1. <span data-ttu-id="fd7b1-620">Açık **Visual Studio Ultimate 2013** seçip **dosya | Yeni | Proje...**  yeni bir çözüm başlatmak için.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-620">Open **Visual Studio Ultimate 2013** and select **File | New | Project...** to start a new solution.</span></span>

    <span data-ttu-id="fd7b1-621">![Yeni proje oluşturma](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "yeni proje oluşturma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-621">![Creating a new project](maintainable-azure-websites-managing-change-and-scale/_static/image79.png "Creating a new project")</span></span>

    <span data-ttu-id="fd7b1-622">*Yeni proje oluşturma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-622">*Creating a new project*</span></span>
2. <span data-ttu-id="fd7b1-623">İçinde **yeni proje** iletişim kutusunda **Web performansı ve yük testi projesi** altında **Visual C# | Test** sekmesi. Emin olun **.NET Framework 4.5** olduğu belirlenirse, projeyi adlandırın *WebAndLoadTestProject*, seçin bir **konumu** tıklatıp **Tamam**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-623">In the **New Project** dialog box, select **Web Performance and Load Test Project** under the **Visual C# | Test** tab. Make sure **.NET Framework 4.5** is selected, name the project *WebAndLoadTestProject*, choose a **Location** and click **OK**.</span></span>

    <span data-ttu-id="fd7b1-624">![Yeni Web ve yük testi projesi oluşturma](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "yeni Web ve yük testi projesi oluşturma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-624">![Creating a new Web and Load Test project](maintainable-azure-websites-managing-change-and-scale/_static/image80.png "Creating a new Web and Load Test project")</span></span>

    <span data-ttu-id="fd7b1-625">*Yeni Web ve yük testi projesi oluşturma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-625">*Creating a new Web and Load Test project*</span></span>
3. <span data-ttu-id="fd7b1-626">İçinde **WebTest1.webtest** sağ **WebTest1'i** düğüm ve tıklatın **ekleme isteği**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-626">In the **WebTest1.webtest** Right-click the **WebTest1** node and click **Add Request**.</span></span>

    <span data-ttu-id="fd7b1-627">![Bir istek için WebTest1'i ekleyerek](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "WebTest1'i için bir istek ekleme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-627">![Adding a request to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image81.png "Adding a request to WebTest1")</span></span>

    <span data-ttu-id="fd7b1-628">*Bir istek için WebTest1'i ekleme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-628">*Adding a request to WebTest1*</span></span>
4. <span data-ttu-id="fd7b1-629">İçinde **özellikleri** penceresi yeni istek düğümünün güncelleştirme **Url** özelliği, web uygulamanızın URL'sine işaret edecek şekilde (örneğin *[ http://geek-quiz.azurewebsites.net/ ](http://geek-quiz.azurewebsites.net/)*).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-629">In the **Properties** window of the new request node, update the **Url** property to point to the URL of your web app (e.g. *[http://geek-quiz.azurewebsites.net/](http://geek-quiz.azurewebsites.net/)*).</span></span>

    <span data-ttu-id="fd7b1-630">![Url özelliğini değiştirme](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Url özelliğini değiştirme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-630">![Changing the Url property](maintainable-azure-websites-managing-change-and-scale/_static/image82.png "Changing the Url property")</span></span>

    <span data-ttu-id="fd7b1-631">*Url özelliğini değiştirme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-631">*Changing the Url property*</span></span>
5. <span data-ttu-id="fd7b1-632">İçinde **WebTest1.webtest** penceresinde sağ **WebTest1'i** tıklatıp **döngü Ekle...** .</span><span class="sxs-lookup"><span data-stu-id="fd7b1-632">In the **WebTest1.webtest** window, right-click **WebTest1** and click **Add Loop...**.</span></span>

    <span data-ttu-id="fd7b1-633">![Bir döngü için WebTest1'i ekleme](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "WebTest1'i için döngü ekleme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-633">![Adding a loop to WebTest1](maintainable-azure-websites-managing-change-and-scale/_static/image83.png "Adding a loop to WebTest1")</span></span>

    <span data-ttu-id="fd7b1-634">*Bir döngü için WebTest1'i ekleme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-634">*Adding a loop to WebTest1*</span></span>
6. <span data-ttu-id="fd7b1-635">İçinde **koşullu Kural Ekle ve döngü öğeler** iletişim kutusunda **For döngüsü** kural ve aşağıdaki özellikleri değiştirebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-635">In the **Add Conditional Rule and Items to Loop** dialog box, select the **For Loop** rule and modify the following properties.</span></span>

   1. <span data-ttu-id="fd7b1-636">**Sonlandırma değeri:** 1000</span><span class="sxs-lookup"><span data-stu-id="fd7b1-636">**Terminating value:** 1000</span></span>
   2. <span data-ttu-id="fd7b1-637">**Bağlam parametresi adı:** yineleyici</span><span class="sxs-lookup"><span data-stu-id="fd7b1-637">**Context Parameter Name:** Iterator</span></span>
   3. <span data-ttu-id="fd7b1-638">**Değeri Artır:** 1</span><span class="sxs-lookup"><span data-stu-id="fd7b1-638">**Increment Value:** 1</span></span>

      <span data-ttu-id="fd7b1-639">![For döngüsü kuralı seçip özelliklerini güncelleştirme](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "For döngüsü kuralı seçip özellikleri güncelleştiriliyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-639">![Selecting the For Loop rule and updating the properties](maintainable-azure-websites-managing-change-and-scale/_static/image84.png "Selecting the For Loop rule and updating the properties")</span></span>

      <span data-ttu-id="fd7b1-640">*For döngüsü kuralı seçip özellikleri güncelleştiriliyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-640">*Selecting the For Loop rule and updating the properties*</span></span>
7. <span data-ttu-id="fd7b1-641">Altında **döngüsel öğeler** bölümünde, daha önce oluşturduğunuz ilk ve son öğe döngü için olmasını isteği seçin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-641">Under the **Items in loop** section, select the request you created previously to be the first and last item for the loop.</span></span> <span data-ttu-id="fd7b1-642">Devam etmek için **Tamam** 'a tıklayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-642">Click **OK** to continue.</span></span>

    <span data-ttu-id="fd7b1-643">![Döngü için ilk ve son öğe seçme](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "döngü için ilk ve son öğe seçme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-643">![Selecting the first and last items for the loop](maintainable-azure-websites-managing-change-and-scale/_static/image85.png "Selecting the first and last items for the loop")</span></span>

    <span data-ttu-id="fd7b1-644">*Döngü için ilk ve son öğe seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-644">*Selecting the first and last items for the loop*</span></span>
8. <span data-ttu-id="fd7b1-645">İçinde **Çözüm Gezgini**, sağ **WebAndLoadTestProject** proje, genişletin **Ekle** menü ve select **yük testi...** .</span><span class="sxs-lookup"><span data-stu-id="fd7b1-645">In **Solution Explorer**, right-click the **WebAndLoadTestProject** project, expand the **Add** menu and select **Load Test...**.</span></span>

    <span data-ttu-id="fd7b1-646">![Bir yük testi WebAndLoadTestProject projeye eklenirken](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "bir yük testi WebAndLoadTestProject projeye ekleniyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-646">![Adding a Load Test to the WebAndLoadTestProject project](maintainable-azure-websites-managing-change-and-scale/_static/image86.png "Adding a Load Test to the WebAndLoadTestProject project")</span></span>

    <span data-ttu-id="fd7b1-647">*Bir yük testi WebAndLoadTestProject projeye ekleniyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-647">*Adding a Load Test to the WebAndLoadTestProject project*</span></span>
9. <span data-ttu-id="fd7b1-648">İçinde **Yeni Yük Testi Sihirbazı** iletişim kutusu, tıklayın **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-648">In the **New Load Test Wizard** dialog box, click **Next**.</span></span>

    <span data-ttu-id="fd7b1-649">![Yeni Yük Testi Sihirbazı](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "Yeni Yük Testi Sihirbazı")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-649">![New Load Test Wizard](maintainable-azure-websites-managing-change-and-scale/_static/image87.png "New Load Test Wizard")</span></span>

    <span data-ttu-id="fd7b1-650">*Yeni Yük Testi Sihirbazı*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-650">*New Load Test Wizard*</span></span>
10. <span data-ttu-id="fd7b1-651">İçinde **senaryo** sayfasında **Düşünme süreleri kullanmayın** tıklatıp **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-651">In the **Scenario** page, select **Do not use think times** and click **Next**.</span></span>

    <span data-ttu-id="fd7b1-652">![Düşünme süreleri kullanmayı seçme](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Düşünme süreleri kullanmayı seçme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-652">![Selecting not to use think times](maintainable-azure-websites-managing-change-and-scale/_static/image88.png "Selecting not to use think times")</span></span>

    <span data-ttu-id="fd7b1-653">*Düşünme süreleri kullanmayı seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-653">*Selecting not to use think times*</span></span>
11. <span data-ttu-id="fd7b1-654">İçinde **yük düzeni** sayfasında, aşağıdakileri sağlayın **sabit yük** seçeneği belirlenir.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-654">In the **Load Pattern** page, make sure that the **Constant Load** option is selected.</span></span> <span data-ttu-id="fd7b1-655">Değişiklik **kullanıcı sayısı** ayarını **250** kullanıcılar ve tıklatın **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-655">Change the **User Count** setting to **250** users and click **Next**.</span></span>

    <span data-ttu-id="fd7b1-656">![Kullanıcı sayısını 250 ile değiştirme](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "kullanıcı sayısını 250 ile değiştirme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-656">![Changing the user count to 250](maintainable-azure-websites-managing-change-and-scale/_static/image89.png "Changing the user count to 250")</span></span>

    <span data-ttu-id="fd7b1-657">*Kullanıcı sayısını 250 ile değiştirme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-657">*Changing the user count to 250*</span></span>
12. <span data-ttu-id="fd7b1-658">İçinde **Test karışımı modeli** sayfasında **sıralı test düzeni tabanlı** tıklatıp **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-658">In the **Test Mix Model** page, select **Based on sequential test order** and click **Next**.</span></span>

    <span data-ttu-id="fd7b1-659">![Test karışımı modelini seçmek](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "test karışımı modelini seçme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-659">![Selecting the test mix model](maintainable-azure-websites-managing-change-and-scale/_static/image90.png "Selecting the test mix model")</span></span>

    <span data-ttu-id="fd7b1-660">*Test karışımı modelini seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-660">*Selecting the test mix model*</span></span>
13. <span data-ttu-id="fd7b1-661">İçinde **Test karışımı modeli** sayfasında **Ekle...**  test karışımına eklenecek.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-661">In the **Test Mix Model** page, click **Add...** to add a test to the mix.</span></span>

    <span data-ttu-id="fd7b1-662">![Bir test için test karışımını eklenmesi](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "bir test için test karışımını ekleme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-662">![Adding a test to the test mix](maintainable-azure-websites-managing-change-and-scale/_static/image91.png "Adding a test to the test mix")</span></span>

    <span data-ttu-id="fd7b1-663">*Bir test için test karışımını ekleme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-663">*Adding a test to the test mix*</span></span>
14. <span data-ttu-id="fd7b1-664">İçinde **Test Ekle** iletişim kutusunda, çift **WebTest1'i** testine ekleme **Seçili testler** listesi.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-664">In the **Add Tests** dialog box, double-click **WebTest1** to add the test to the **Selected tests** list.</span></span> <span data-ttu-id="fd7b1-665">Devam etmek için **Tamam** 'a tıklayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-665">Click **OK** to continue.</span></span>

    <span data-ttu-id="fd7b1-666">![WebTest1'i test eklenmesi](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "WebTest1'i test ekleme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-666">![Adding the WebTest1 test](maintainable-azure-websites-managing-change-and-scale/_static/image92.png "Adding the WebTest1 test")</span></span>

    <span data-ttu-id="fd7b1-667">*WebTest1'i test ekleme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-667">*Adding the WebTest1 test*</span></span>
15. <span data-ttu-id="fd7b1-668">Geri **Test Karışımını** sayfasında **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-668">Back in the **Test Mix** page, click **Next**.</span></span>

    <span data-ttu-id="fd7b1-669">![Test karışımı sayfasını tamamladıktan](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Test Karışımını sayfanın Tamamlanıyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-669">![Completing the Test Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image93.png "Completing the Test Mix page")</span></span>

    <span data-ttu-id="fd7b1-670">*Test Karışımı sayfası Tamamlanıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-670">*Completing the Test Mix page*</span></span>
16. <span data-ttu-id="fd7b1-671">İçinde **ağ karışımını** sayfasında **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-671">In the **Network Mix** page, click **Next**.</span></span>

    <span data-ttu-id="fd7b1-672">![Ağ karışımı sayfasında İleri seçeneğine tıkladığınızda](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "ağ karışımını sayfasında İleri seçeneğine tıkladığınızda")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-672">![Clicking next in the Network Mix page](maintainable-azure-websites-managing-change-and-scale/_static/image94.png "Clicking next in the Network Mix page")</span></span>

    <span data-ttu-id="fd7b1-673">*Ağ karışımı sayfasında İleri'ye tıklama*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-673">*Clicking next in the Network Mix page*</span></span>
17. <span data-ttu-id="fd7b1-674">İçinde **tarayıcı karışımı** sayfasında **Internet Explorer 10.0** tıklayın ve tarayıcı türü olarak **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-674">In the **Browser Mix** page, select **Internet Explorer 10.0** as the browser type and click **Next**.</span></span>

    <span data-ttu-id="fd7b1-675">![Tarayıcı türü seçme](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "tarayıcı türü seçme")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-675">![Selecting the browser type](maintainable-azure-websites-managing-change-and-scale/_static/image95.png "Selecting the browser type")</span></span>

    <span data-ttu-id="fd7b1-676">*Tarayıcı türü seçme*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-676">*Selecting the browser type*</span></span>
18. <span data-ttu-id="fd7b1-677">İçinde **sayaç kümeleri** sayfasında **sonraki**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-677">In the **Counter Sets** page, click **Next**.</span></span>

    <span data-ttu-id="fd7b1-678">![Sayaç kümeleri sayfasında İleri'ye tıklama](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "sayaç kümeleri sayfasında İleri tıklayarak")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-678">![Clicking Next in the Counter Sets page](maintainable-azure-websites-managing-change-and-scale/_static/image96.png "Clicking Next in the Counter Sets page")</span></span>

    <span data-ttu-id="fd7b1-679">*Sayaç kümeleri sayfasında İleri'ye tıklama*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-679">*Clicking Next in the Counter Sets page*</span></span>
19. <span data-ttu-id="fd7b1-680">İçinde **çalıştırma ayarları** sayfasında **yük testi süresi** için **5 dakika** tıklatıp **son**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-680">In the **Run Settings** page, set the **Load test duration** to **5 minutes** and click **Finish**.</span></span>

    <span data-ttu-id="fd7b1-681">![Yük testi süresi 5 dakikaya ayarlanıyor](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "yük testi süresi 5 dakikaya ayarlanıyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-681">![Setting the load test duration to 5 minutes](maintainable-azure-websites-managing-change-and-scale/_static/image97.png "Setting the load test duration to 5 minutes")</span></span>

    <span data-ttu-id="fd7b1-682">*Yük testi süresi 5 dakikaya ayarlanıyor*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-682">*Setting the load test duration to 5 minutes*</span></span>
20. <span data-ttu-id="fd7b1-683">İçinde **Çözüm Gezgini**, çift **Local.settings** dosyasının test ayarları keşfedin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-683">In **Solution Explorer**, double-click the **Local.settings** file to explore the test settings.</span></span> <span data-ttu-id="fd7b1-684">Varsayılan olarak, Visual Studio, testleri çalıştırmak için yerel bilgisayarınıza kullanır.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-684">By default, Visual Studio uses your local computer to run the tests.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-685">Alternatif olarak, Bulutu kullanarak yük testleri çalıştırmak için test projenizi yapılandırabilirsiniz **Azure Test planları**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-685">Alternatively, you can configure your test project to run the load tests in the cloud using **Azure Test Plans**.</span></span> <span data-ttu-id="fd7b1-686">Azure Test planlarını daha gerçekçi bir yükü benzetir hizmeti test etmek, CPU kapasitesi, kullanılabilir bellek ve ağ bant genişliği gibi yerel ortam kısıtlamalarını önleme bir bulut tabanlı yük sağlar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-686">Azure Test Plans provides a cloud-based load testing service that simulates a more realistic load, avoiding local environment constraints like CPU capacity, available memory, and network bandwidth.</span></span> <span data-ttu-id="fd7b1-687">Yük testleri çalıştırmak için Azure Test planlarını kullanma hakkında daha fazla bilgi için bkz. [yük testi senaryoları](/azure/devops/test/load-test/overview?view=vsts).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-687">For more information about using Azure Test Plans to run load tests, see [Load testing scenarios](/azure/devops/test/load-test/overview?view=vsts).</span></span>

    ![Test ayarları](maintainable-azure-websites-managing-change-and-scale/_static/image98.png)

<a id="Ex5Task3"></a>
#### <a name="task-3--autoscale-verification"></a><span data-ttu-id="fd7b1-689">Görev 3: otomatik ölçeklendirme doğrulama</span><span class="sxs-lookup"><span data-stu-id="fd7b1-689">Task 3 – Autoscale Verification</span></span>

<span data-ttu-id="fd7b1-690">Şimdi, önceki görevde oluşturduğunuz yük testi yürütmek ve web uygulamanızı yük altında nasıl davrandığını görmek.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-690">You will now execute the load test you created in the previous task and see how your web app behaves under load.</span></span>

1. <span data-ttu-id="fd7b1-691">İçinde **Çözüm Gezgini**, çift **LoadTest1.loadtest** yük testi açın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-691">In **Solution Explorer**, double-click **LoadTest1.loadtest** to open the load test.</span></span>

    <span data-ttu-id="fd7b1-692">![LoadTest1.loadtest açma](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "LoadTest1.loadtest açma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-692">![Opening LoadTest1.loadtest](maintainable-azure-websites-managing-change-and-scale/_static/image99.png "Opening LoadTest1.loadtest")</span></span>

    <span data-ttu-id="fd7b1-693">*Açılış LoadTest1.loadtest*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-693">*Opening LoadTest1.loadtest*</span></span>
2. <span data-ttu-id="fd7b1-694">İçinde **LoadTest1.loadtest** penceresinde yük testini çalıştırmak için araç kutusunda ilk düğmesine tıklayın.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-694">In the **LoadTest1.loadtest** window, click the first button in the toolbox to run the load test.</span></span>

    <span data-ttu-id="fd7b1-695">![Yük testi çalıştırma](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "yük testi çalıştırma")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-695">![Running the load test](maintainable-azure-websites-managing-change-and-scale/_static/image100.png "Running the load test")</span></span>

    <span data-ttu-id="fd7b1-696">*Yük testi çalıştırma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-696">*Running the load test*</span></span>
3. <span data-ttu-id="fd7b1-697">Yük testi tamamlanana kadar bekleyin.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-697">Wait until the load test completes.</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-698">Yük testi, web uygulamasına isteği gönderen, aynı anda birden çok kullanıcının benzetimini yapar.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-698">The load test simulates multiple users that send requests to the web app simultaneously.</span></span> <span data-ttu-id="fd7b1-699">Bir test çalıştırırken, hataları, uyarıları veya yük test ile ilgili diğer bilgileri algılamak için kullanılabilir sayaçları izleyebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-699">When the test is running, you can monitor the available counters to detect any errors, warnings or other information related to your load test run.</span></span>

    <span data-ttu-id="fd7b1-700">![Yük testi çalıştıran](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "yük testi tamamlanana kadar bekleniyor")</span><span class="sxs-lookup"><span data-stu-id="fd7b1-700">![Load test running](maintainable-azure-websites-managing-change-and-scale/_static/image101.png "Waiting until the load test completes")</span></span>

    <span data-ttu-id="fd7b1-701">*Yük testi çalıştırma*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-701">*Load test running*</span></span>
4. <span data-ttu-id="fd7b1-702">Test tamamlandıktan sonra Yönetim Portalı'na geri dönün ve gidin **ölçek** web uygulamanızın sayfasında.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-702">Once the test completes, go back to the management portal and navigate to the **Scale** page of your web app.</span></span> <span data-ttu-id="fd7b1-703">Altında **kapasite** bölümünde, yeni bir örneği otomatik olarak dağıtılan grafikte görmelisiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-703">Under the **capacity** section, you should see in the graph that a new instance was automatically deployed.</span></span>

    ![Otomatik olarak dağıtılan yeni örneği](maintainable-azure-websites-managing-change-and-scale/_static/image102.png)

    <span data-ttu-id="fd7b1-705">*Otomatik olarak dağıtılan yeni örneği*</span><span class="sxs-lookup"><span data-stu-id="fd7b1-705">*New instance automatically deployed*</span></span>

    > [!NOTE]
    > <span data-ttu-id="fd7b1-706">Değişikliklerin grafikte görünmesi birkaç dakika sürebilir (basın **CTRL + F5 tuşlarına basarak** düzenli aralıklarla sayfayı yenilemek için).</span><span class="sxs-lookup"><span data-stu-id="fd7b1-706">It may take several minutes for the changes to appear in the graph (press **CTRL + F5** periodically to refresh the page).</span></span> <span data-ttu-id="fd7b1-707">Herhangi bir değişiklik görmüyorsanız, aşağıdakileri deneyebilirsiniz:</span><span class="sxs-lookup"><span data-stu-id="fd7b1-707">If you do not see any changes, you can try the following:</span></span>
    >
    > - <span data-ttu-id="fd7b1-708">Yük testi süresini artırmak (örneğin için **10 dakika**)</span><span class="sxs-lookup"><span data-stu-id="fd7b1-708">Increase the duration of the load test (e.g. to **10 minutes**)</span></span>
    > - <span data-ttu-id="fd7b1-709">Maksimum ve minimum değerleri azaltmak **hedef CPU** aralığında web uygulamanız otomatik ölçeklendirme yapılandırması</span><span class="sxs-lookup"><span data-stu-id="fd7b1-709">Reduce the maximum and minimum values of the **Target CPU** range in the Autoscale configuration of your web app</span></span>
    > - <span data-ttu-id="fd7b1-710">Bulutta yük testi çalıştırma **Azure Test planları**.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-710">Run the load test in the cloud with **Azure Test Plans**.</span></span> <span data-ttu-id="fd7b1-711">Daha fazla bilgi [burada](/azure/devops/test/load-test/index?view=vsts)</span><span class="sxs-lookup"><span data-stu-id="fd7b1-711">More information [here](/azure/devops/test/load-test/index?view=vsts)</span></span>

* * *

<a id="Summary"></a>
## <a name="summary"></a><span data-ttu-id="fd7b1-712">Özet</span><span class="sxs-lookup"><span data-stu-id="fd7b1-712">Summary</span></span>

<span data-ttu-id="fd7b1-713">Bu uygulamalı laboratuvarda ayarlama ve uygulamanızı üretim web uygulamalarını azure'da dağıtma hakkında bilgi edindiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-713">In this hands-on lab, you learned how to set up and deploy your application to production web apps in Azure.</span></span> <span data-ttu-id="fd7b1-714">Algılama ve veritabanlarınızı kullanarak güncelleştirerek başlattığınız **Entity Framework Code First Migrations**, ardından yeni sürümlerini kullanarak siteyi dağıtarak devam **Git** ve düzeyine gerçekleştirme sitenizi en son kararlı sürümü.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-714">You started by detecting and updating your databases using **Entity Framework Code First Migrations**, then continued by deploying new versions of your site using **Git** and performing rollbacks to the latest stable version of your site.</span></span> <span data-ttu-id="fd7b1-715">Ayrıca, statik içerik Blob kapsayıcısına taşımak için depolama kullanarak uygulamanızı ölçeklendirme hakkında bilgi edindiniz.</span><span class="sxs-lookup"><span data-stu-id="fd7b1-715">Additionally, you learned how to scale your app using Storage to move your static content to a Blob container.</span></span>
