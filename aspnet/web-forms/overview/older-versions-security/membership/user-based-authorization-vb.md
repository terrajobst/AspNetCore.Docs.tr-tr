...
> [!NOTE]
> `DirectoryInfo` Ve `FileInfo` içinde bulunan sınıflar [ `System.IO` ad alanı](https://msdn.microsoft.com/en-us/library/system.io.aspx). Bu nedenle, bu sınıf adları ad alanı adları ile yazdığınızdan veya sınıf dosyasına içeri aktarılan ad alanınız için her iki gereksinim olur (aracılığıyla `Imports System.IO`).
