...
- <strong>Bir veya daha yeni Oluştur</strong><strong>`WizardStep`</strong><strong>ek kullanıcı arabirimi öğeleri içerecek şekilde s</strong>. Yeni bir eklemek için `WizardStep` CreateUserWizard için tıklatın "Ekle/Kaldır `WizardSteps`" Akıllı başlatmak için etiketini bağlantısından `WizardStep` Koleksiyonu Düzenleyicisi. Buradan eklemek, kaldırmak veya yeniden sıralamak ve sihirbazdaki adımları. Bu, Bu öğretici için kullanacağız yaklaşımdır.
