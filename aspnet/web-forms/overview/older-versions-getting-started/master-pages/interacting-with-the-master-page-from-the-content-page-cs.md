...
Tüm ASP.NET web sayfaları öğesinden türetilmelidir `Page` bulunan sınıfı `System.Web.UI` ad alanı. `Page` Sınıfı içeren bir [ `Master` özelliği](https://msdn.microsoft.com/en-us/library/system.web.ui.page.master.aspx) sayfasının ana sayfasında bir başvuru döndürür. Sayfa bir ana sayfa yoksa `Master` döndürür `null`.
