---
uid: web-pages/videos/aspnet-razor-pages/creating-a-consistent-look-part-1
title: Tutarlı bir görünüm (Kısım 1) oluşturma | Microsoft Docs
author: JoeStagner
description: Video accompaniment oluşturma için bir tutarlı görünecek öğretici (Kısım 1)
ms.author: aspnetcontent
manager: wpickett
ms.date: 02/25/2011
ms.topic: article
ms.assetid: 1057d00f-187f-4187-a674-194b6262c3a3
ms.technology: dotnet-webpages
ms.prod: .net-framework
msc.legacyurl: /web-pages/videos/aspnet-razor-pages/creating-a-consistent-look-part-1
msc.type: video
ms.openlocfilehash: 8b5e77b87fd17f66bb7c680bbe2ffe8a802de4b6
ms.sourcegitcommit: f8852267f463b62d7f975e56bea9aa3f68fbbdeb
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/06/2018
ms.locfileid: "30893488"
---
<a name="creating-a-consistent-look-part-1"></a>Tutarlı bir görünüm (Kısım 1) oluşturma
====================
tarafından [CAN Stagner](https://github.com/JoeStagner)

Video accompaniment oluşturma için bir tutarlı görünecek öğretici (Kısım 1)

[&#9654;(23 dakika) videoyu izleyin](https://channel9.msdn.com/Blogs/ASP-NET-Site-Videos/creating-a-consistent-look-part-1)

> [!div class="step-by-step"]
> [Önceki](introduction-to-aspnet-web-programming-using-the-razor-syntax.md)
> [sonraki](creating-a-consistent-look-part-2.md)
