...
- [<span data-ttu-id="0046a-117">Web yayımlama geliştirmeleri</span><span class="sxs-lookup"><span data-stu-id="0046a-117">Web Publish Enhancements</span></span>](#publish)
...
<span data-ttu-id="0046a-134">ASP.NET ve Web Araçları, Visual Studio 2013 gerektirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-134">ASP.NET and Web Tools requires Visual Studio 2013.</span></span>
...
### <a name="aspnet-identity"></a><span data-ttu-id="0046a-224">ASP.NET Kimlik</span><span class="sxs-lookup"><span data-stu-id="0046a-224">ASP.NET Identity</span></span>
...
### <a name="filter-overrides"></a><span data-ttu-id="0046a-234">Filtresi geçersiz kılmaları</span><span class="sxs-lookup"><span data-stu-id="0046a-234">Filter overrides</span></span>
...
- <span data-ttu-id="0046a-273">Bir OData hizmeti $batch uç noktasını kullanın.</span><span class="sxs-lookup"><span data-stu-id="0046a-273">Use the $batch endpoint of an OData service.</span></span>
...
<span data-ttu-id="0046a-344">İçinde bir **uygulama barındırabileceğiniz**, başlangıç sınıfı türü parametresi olarak geçirilir `WebApp.Start` yöntemi.</span><span class="sxs-lookup"><span data-stu-id="0046a-344">In a **Self-hosted application**, the Startup class is passed as the type parameter of the `WebApp.Start` method.</span></span>
...
<span data-ttu-id="0046a-354">SignalR 2.0 sürümünde yeni bir CORS ara yazılım eklemek için Ekle `Microsoft.Owin.Cors` kitaplık projesi ve çağrı `UseCors` bölümünde gösterildiği gibi SignalR ara yazılım önce.</span><span class="sxs-lookup"><span data-stu-id="0046a-354">To add the new CORS middleware in SignalR 2.0, add the `Microsoft.Owin.Cors` library to your project, and call `UseCors` before your SignalR middleware, as shown in the section below.</span></span>
...
<span data-ttu-id="0046a-403">**.NET istemci kodu gösteren sunucudan gönderilen bir HubException yanıtlama**</span><span class="sxs-lookup"><span data-stu-id="0046a-403">**.NET client code demonstrating responding to a HubException sent from the server**</span></span>

[!code-csharp[Main](release-notes/samples/sample17.cs)]

<a id="unittesting"></a>

### <a name="easier-unit-testing-of-hubs"></a><span data-ttu-id="0046a-404">Daha kolay birim test hub'ları</span><span class="sxs-lookup"><span data-stu-id="0046a-404">Easier unit testing of hubs</span></span>

<span data-ttu-id="0046a-405">SignalR 2.0 içerir adlı bir arabirim `IHubCallerConnectionContext` sahte istemci tarafı çağrılarını oluşturmayı kolaylaştırması hub'ları üzerinde.</span><span class="sxs-lookup"><span data-stu-id="0046a-405">SignalR 2.0 includes an interface called `IHubCallerConnectionContext` on Hubs that makes it easier to create mock client side invocations.</span></span> <span data-ttu-id="0046a-406">Popüler test harnesses ile bu arabirimi kullanılarak aşağıdaki kod parçacıkları göstermek [xUnit.net](https://github.com/xunit/xunit) ve [moq](https://code.google.com/p/moq/).</span><span class="sxs-lookup"><span data-stu-id="0046a-406">The following code snippets demonstrate using this interface with popular test harnesses [xUnit.net](https://github.com/xunit/xunit) and [moq](https://code.google.com/p/moq/).</span></span>

<span data-ttu-id="0046a-407">**Birim SignalR ile xUnit.net testi**</span><span class="sxs-lookup"><span data-stu-id="0046a-407">**Unit testing SignalR with xUnit.net**</span></span>

[!code-csharp[Main](release-notes/samples/sample18.cs)]

<span data-ttu-id="0046a-408">**Birim SignalR ile moq testi**</span><span class="sxs-lookup"><span data-stu-id="0046a-408">**Unit testing SignalR with moq**</span></span>

[!code-csharp[Main](release-notes/samples/sample19.cs)]

<a id="javascripterror"></a>

### <a name="javascript-error-handling"></a><span data-ttu-id="0046a-409">JavaScript hata işleme</span><span class="sxs-lookup"><span data-stu-id="0046a-409">JavaScript error handling</span></span>

<span data-ttu-id="0046a-410">SignalR 2. 0'da, tüm JavaScript hata işleme geri çağırmaları ham dizeler yerine JavaScript hata nesneleri döndürür.</span><span class="sxs-lookup"><span data-stu-id="0046a-410">In SignalR 2.0, all JavaScript error handling callbacks return JavaScript error objects instead of raw strings.</span></span> <span data-ttu-id="0046a-411">Bu SignalR hata İşleyicileriniz için daha zengin bilgi akışına olanak sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-411">This allows SignalR to flow richer information to your error handlers.</span></span> <span data-ttu-id="0046a-412">İç özel durumu alabilirsiniz `source` hata özelliğidir.</span><span class="sxs-lookup"><span data-stu-id="0046a-412">You can get the inner exception from the `source` property of the error.</span></span>

<span data-ttu-id="0046a-413">**JavaScript istemci Start.Fail özel durumu işleyen bir kodu**</span><span class="sxs-lookup"><span data-stu-id="0046a-413">**JavaScript client code that handles the Start.Fail exception**</span></span>

[!code-javascript[Main](release-notes/samples/sample20.js)]

<a id="TOC8"></a>
## <a name="aspnet-identity"></a><span data-ttu-id="0046a-414">ASP.NET Kimlik</span><span class="sxs-lookup"><span data-stu-id="0046a-414">ASP.NET Identity</span></span>

### <a name="new-aspnet-membership-system"></a><span data-ttu-id="0046a-415">Yeni ASP.NET üyelik sistemi</span><span class="sxs-lookup"><span data-stu-id="0046a-415">New ASP.NET Membership System</span></span>

<span data-ttu-id="0046a-416">ASP.NET, ASP.NET uygulamaları için yeni üyelik sistemini kimliğidir.</span><span class="sxs-lookup"><span data-stu-id="0046a-416">ASP.NET Identity is the new membership system for ASP.NET applications.</span></span> <span data-ttu-id="0046a-417">ASP.NET Identity kullanıcı özel profil verileri uygulama verileriyle tümleştirmeyi kolaylaştırır.</span><span class="sxs-lookup"><span data-stu-id="0046a-417">ASP.NET Identity makes it easy to integrate user-specific profile data with application data.</span></span> <span data-ttu-id="0046a-418">ASP.NET Identity kullanıcı profilleri için Kalıcılık modeli uygulamanızda seçmenize olanak sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-418">ASP.NET Identity also allows you to choose the persistence model for user profiles in your application.</span></span> <span data-ttu-id="0046a-419">SQL Server veritabanı veya Azure depolama tabloları gibi NoSQL veri deposu dahil olmak üzere başka bir veri deposuna verileri depolayabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="0046a-419">You can store the data in a SQL Server database or another data store, including NoSQL data stores such as Azure Storage Tables.</span></span> <span data-ttu-id="0046a-420">Daha fazla bilgi için [bireysel kullanıcı hesapları](creating-web-projects-in-visual-studio.md#indauth) içinde **Visual Studio 2013'te ASP.NET Web projeleri oluşturma**.</span><span class="sxs-lookup"><span data-stu-id="0046a-420">For more information, see [Individual User Accounts](creating-web-projects-in-visual-studio.md#indauth) in **Creating ASP.NET Web Projects in Visual Studio 2013**.</span></span>

### <a name="claims-based-authentication"></a><span data-ttu-id="0046a-421">Beyana dayalı kimlik doğrulama</span><span class="sxs-lookup"><span data-stu-id="0046a-421">Claims-based authentication</span></span>

<span data-ttu-id="0046a-422">ASP.NET, şimdi burada kullanıcının kimliğini güvenilen veren gelen talepler kümesi olarak temsil edilen talep tabanlı kimlik doğrulaması, destekler.</span><span class="sxs-lookup"><span data-stu-id="0046a-422">ASP.NET now supports claims-based authentication, where the user's identity is represented as a set of claims from a trusted issuer.</span></span> <span data-ttu-id="0046a-423">Kullanıcılar, kimliği doğrulanmış bir kullanıcı adı ve parolayı bir uygulamanın veritabanında tutulan veya sosyal kimlik sağlayıcılarını kullanarak olabilir (örneğin: Microsoft Accounts, Facebook, Google, Twitter), veya Azure Active Directory aracılığıyla Kurumsal hesaplarını kullanan veya Active Directory Federasyon Hizmetleri (ADFS).</span><span class="sxs-lookup"><span data-stu-id="0046a-423">Users can be authenticated using a username and password maintained in an application database, or using social identity providers (for example: Microsoft Accounts, Facebook, Google, Twitter), or using organizational accounts through Azure Active Directory or Active Directory Federation Services (ADFS).</span></span>

### <a name="integration-with-azure-active-directory-and-windows-server-active-directory"></a><span data-ttu-id="0046a-424">Azure Active Directory ve Windows Server Active Directory ile tümleştirme</span><span class="sxs-lookup"><span data-stu-id="0046a-424">Integration with Azure Active Directory and Windows Server Active Directory</span></span>

<span data-ttu-id="0046a-425">Artık Azure Active Directory veya Windows Server Active Directory (AD) için kimlik doğrulaması kullanan ASP.NET projeleri oluşturabilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="0046a-425">You can now create ASP.NET projects that use Azure Active Directory or Windows Server Active Directory (AD) for authentication.</span></span> <span data-ttu-id="0046a-426">Daha fazla bilgi için [Kurumsal hesaplar](creating-web-projects-in-visual-studio.md#orgauth) içinde **Visual Studio 2013'te ASP.NET Web projeleri oluşturma**.</span><span class="sxs-lookup"><span data-stu-id="0046a-426">For more information, see [Organizational Accounts](creating-web-projects-in-visual-studio.md#orgauth) in **Creating ASP.NET Web Projects in Visual Studio 2013**.</span></span>

### <a name="owin-integration"></a><span data-ttu-id="0046a-427">OWIN tümleştirme</span><span class="sxs-lookup"><span data-stu-id="0046a-427">OWIN Integration</span></span>

<span data-ttu-id="0046a-428">ASP.NET kimlik doğrulaması, artık tüm OWIN tabanlı ana bilgisayarda kullanılan OWIN ara yazılımı dayanır.</span><span class="sxs-lookup"><span data-stu-id="0046a-428">ASP.NET authentication is now based on OWIN middleware that can be used on any OWIN-based host.</span></span> <span data-ttu-id="0046a-429">OWIN hakkında daha fazla bilgi için aşağıdakilere bakın [Microsoft OWIN bileşenleri](#TOC7) bölümü.</span><span class="sxs-lookup"><span data-stu-id="0046a-429">For more information about OWIN, see the following [Microsoft OWIN Components](#TOC7) section.</span></span>

<a id="TOC7"></a>
## <a name="microsoft-owin-components"></a><span data-ttu-id="0046a-430">Microsoft OWIN bileşenleri</span><span class="sxs-lookup"><span data-stu-id="0046a-430">Microsoft OWIN Components</span></span>

<span data-ttu-id="0046a-431">[.NET için açık Web arabirimi](http://owin.org/) (OWIN) .NET web sunucuları ve web uygulaması arasında bir Özet tanımlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-431">[Open Web Interface for .NET](http://owin.org/) (OWIN) defines an abstraction between .NET web servers and web applications.</span></span> <span data-ttu-id="0046a-432">OWIN sunucusu web uygulamasından web uygulamaları konak belirsiz hale ayırır.</span><span class="sxs-lookup"><span data-stu-id="0046a-432">OWIN decouples the web application from the server, making web applications host-agnostic.</span></span> <span data-ttu-id="0046a-433">Örneğin, bir OWIN tabanlı web uygulamasının IIS'deki konak veya özel bir işlemde kendiliğinden konak.</span><span class="sxs-lookup"><span data-stu-id="0046a-433">For example, you can host an OWIN-based web application in IIS or self-host it in a custom process.</span></span>

<span data-ttu-id="0046a-434">Microsoft OWIN bileşenleri (Katana proje olarak da bilinir) sunulan değişiklikler, yeni sunucu ve ana bileşenleri, yeni Yardımcısı kitaplıkları ve ara yazılım ve yeni kimlik doğrulaması ara yazılımı içerir.</span><span class="sxs-lookup"><span data-stu-id="0046a-434">Changes introduced in the Microsoft OWIN components (also known as the Katana project) include new server and host components, new helper libraries and middleware, and new authentication middleware.</span></span>

<span data-ttu-id="0046a-435">OWIN ve Katana hakkında daha fazla bilgi için bkz: [OWIN ve Katana yenilikler](../../../aspnet/overview/owin-and-katana/index.md).</span><span class="sxs-lookup"><span data-stu-id="0046a-435">For more information about OWIN and Katana, see [What's new in OWIN and Katana](../../../aspnet/overview/owin-and-katana/index.md).</span></span>

<span data-ttu-id="0046a-436">**Not: [OWIN](../../../aspnet/overview/owin-and-katana/an-overview-of-project-katana.md) uygulamalar, IIS Klasik modda çalışamaz; tümleşik modunda çalıştırılmalıdır.**</span><span class="sxs-lookup"><span data-stu-id="0046a-436">**Note: [OWIN](../../../aspnet/overview/owin-and-katana/an-overview-of-project-katana.md) applications cannot run in IIS classic mode; they must be run in integrated mode.**</span></span>

<span data-ttu-id="0046a-437">**Not: [OWIN](../../../aspnet/overview/owin-and-katana/an-overview-of-project-katana.md) uygulamalarını tam güvende çalıştırılması gerekir.**</span><span class="sxs-lookup"><span data-stu-id="0046a-437">**Note: [OWIN](../../../aspnet/overview/owin-and-katana/an-overview-of-project-katana.md) applications must be run in full trust.**</span></span>

### <a name="new-servers-and-hosts"></a><span data-ttu-id="0046a-438">Yeni sunucular ve konakları</span><span class="sxs-lookup"><span data-stu-id="0046a-438">New Servers and Hosts</span></span>

<span data-ttu-id="0046a-439">Bu sürümle birlikte, yeni bileşenler barındırma senaryolarını etkinleştirmek için eklendi.</span><span class="sxs-lookup"><span data-stu-id="0046a-439">With this release, new components were added to enable self-host scenarios.</span></span> <span data-ttu-id="0046a-440">Bu bileşenleri aşağıdaki NuGet paketlerini içerir:</span><span class="sxs-lookup"><span data-stu-id="0046a-440">These components include the following NuGet packages:</span></span>

- <span data-ttu-id="0046a-441">**Microsoft.Owin.Host.HttpListener**.</span><span class="sxs-lookup"><span data-stu-id="0046a-441">**Microsoft.Owin.Host.HttpListener**.</span></span> <span data-ttu-id="0046a-442">Kullanan bir OWIN sunucusu sağlar **HttpListener** HTTP isteklerini dinlemek ve bunları OWIN ardışık düzenine yönlendirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-442">Provides an OWIN server that uses **HttpListener** to listen for HTTP requests and direct them into the OWIN pipeline.</span></span>
- <span data-ttu-id="0046a-443">**Microsoft.Owin.Hosting** bir OWIN ardışık düzenine bir konsol uygulaması veya Windows hizmeti gibi özel bir işlemde barındırılan isteyen geliştiriciler için bir kitaplık sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-443">**Microsoft.Owin.Hosting** Provides a library for developers who wish to self-host an OWIN pipeline in a custom process, such as a console application or Windows service.</span></span>
- <span data-ttu-id="0046a-444">**OwinHost**.</span><span class="sxs-lookup"><span data-stu-id="0046a-444">**OwinHost**.</span></span> <span data-ttu-id="0046a-445">Sarmalayan bir tek başına yürütülebilir dosya sağlar `Microsoft.Owin.Hosting` ve bir OWIN ardışık düzenini özel bir Windows uygulaması yazmak zorunda kalmadan Self barındırmanıza olanak tanır.</span><span class="sxs-lookup"><span data-stu-id="0046a-445">Provides a stand-alone executable that wraps `Microsoft.Owin.Hosting` and lets you self-host an OWIN pipeline without having to write a custom host application.</span></span>

<span data-ttu-id="0046a-446">Ayrıca, `Microsoft.Owin.Host.SystemWeb` paket artık derleyiciye sağlamak bir ara yazılım sağlar **SystemWeb** belirli ASP.NET ardışık düzen aşamaları sırasında ara yazılımın çağrılması gerektiğini belirten bir sunucu.</span><span class="sxs-lookup"><span data-stu-id="0046a-446">In addition, the `Microsoft.Owin.Host.SystemWeb` package now enables middleware to provide hints to the **SystemWeb** server, indicating that the middleware should be called during a specific ASP.NET pipeline stage.</span></span> <span data-ttu-id="0046a-447">Bu özellik, erken ASP.NET işlem hattı çalışması gereken kimlik doğrulaması ara yazılımı için özellikle yararlıdır.</span><span class="sxs-lookup"><span data-stu-id="0046a-447">This feature is particularly useful for authentication middleware, which should run early in the ASP.NET pipeline.</span></span>

### <a name="helper-libraries-and-middleware"></a><span data-ttu-id="0046a-448">Yardımcısı kitaplıkları ve ara yazılım</span><span class="sxs-lookup"><span data-stu-id="0046a-448">Helper Libraries and Middleware</span></span>

<span data-ttu-id="0046a-449">OWIN belirtiminden yalnızca işlevi ve türü tanımlarını kullanarak OWIN bileşenlerini yazabilirsiniz, ancak yeni `Microsoft.Owin` paket soyutlama daha kullanıcı dostu kümesi sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-449">Although you can write OWIN components using only the function and type definitions from the OWIN specification, the new `Microsoft.Owin` package provides a more user-friendly set of abstractions.</span></span> <span data-ttu-id="0046a-450">Bu paketi çeşitli önceki paketler birleştirir (örneğin, `Owin.Extensions`, `Owin.Types`) sonra kolayca diğer OWIN bileşenleri tarafından kullanılabilecek bir tek ve iyi yapılandırılmış nesne modelini.</span><span class="sxs-lookup"><span data-stu-id="0046a-450">This package combines several earlier packages (e.g., `Owin.Extensions`, `Owin.Types`) into a single, well-structured object model that can then be easily used by other OWIN components.</span></span> <span data-ttu-id="0046a-451">Aslında, Microsoft OWIN bileşenleri çoğunu artık kullanın Bu paket.</span><span class="sxs-lookup"><span data-stu-id="0046a-451">In fact, the majority of Microsoft OWIN components now use this package.</span></span>

> [!NOTE]
> <span data-ttu-id="0046a-452">[OWIN](http://www.owin.org) uygulamalar, IIS Klasik modda çalışamaz; tümleşik modunda çalıştırılmalıdır.</span><span class="sxs-lookup"><span data-stu-id="0046a-452">[OWIN](http://www.owin.org) applications cannot run in IIS classic mode; they must be run in integrated mode.</span></span>

> [!NOTE]
> <span data-ttu-id="0046a-453">[OWIN](http://www.owin.org) uygulamalarını tam güvende çalıştırılması gerekir.</span><span class="sxs-lookup"><span data-stu-id="0046a-453">[OWIN](http://www.owin.org) applications must be run in full trust.</span></span>

<span data-ttu-id="0046a-454">Bu sürüm ayrıca hataları araştırmanıza yardımcı olması için hata sayfası ara yazılımını yanı sıra çalışan bir OWIN uygulama doğrulamak için bir ara yazılım içeren Microsoft.Owin.Diagnostics paketini içerir.</span><span class="sxs-lookup"><span data-stu-id="0046a-454">This release also includes the Microsoft.Owin.Diagnostics package, which includes middleware to validate a running OWIN application, plus error-page middleware to help investigate failures.</span></span>

### <a name="authentication-components"></a><span data-ttu-id="0046a-455">Kimlik doğrulama bileşenleri</span><span class="sxs-lookup"><span data-stu-id="0046a-455">Authentication Components</span></span>

<span data-ttu-id="0046a-456">Aşağıdaki kimlik doğrulama bileşenleri kullanılabilir.</span><span class="sxs-lookup"><span data-stu-id="0046a-456">The following authentication components are available.</span></span>

- <span data-ttu-id="0046a-457">**Microsoft.Owin.Security.ActiveDirectory**.</span><span class="sxs-lookup"><span data-stu-id="0046a-457">**Microsoft.Owin.Security.ActiveDirectory**.</span></span> <span data-ttu-id="0046a-458">Şirket içi veya bulut tabanlı dizin hizmetlerini kullanarak kimlik doğrulamasını etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-458">Enables authentication using on-premise or cloud-based directory services.</span></span>
- <span data-ttu-id="0046a-459">**Microsoft.Owin.Security.Cookies** tanımlama bilgilerini kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-459">**Microsoft.Owin.Security.Cookies** Enables authentication using cookies.</span></span> <span data-ttu-id="0046a-460">Bu paket daha önce adlı `Microsoft.Owin.Security.Forms`.</span><span class="sxs-lookup"><span data-stu-id="0046a-460">This package was previously named `Microsoft.Owin.Security.Forms`.</span></span>
- <span data-ttu-id="0046a-461">**Microsoft.Owin.Security.Facebook** Facebook'ın OAuth tabanlı hizmetini kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-461">**Microsoft.Owin.Security.Facebook** Enables authentication using Facebook's OAuth-based service.</span></span>
- <span data-ttu-id="0046a-462">**Microsoft.Owin.Security.Google** Google Openıd tabanlı hizmetini kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-462">**Microsoft.Owin.Security.Google** Enables authentication using Google's OpenID-based service.</span></span>
- <span data-ttu-id="0046a-463">**Microsoft.Owin.Security.Jwt** JWT belirteçleri kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-463">**Microsoft.Owin.Security.Jwt** Enables authentication using JWT tokens.</span></span>
- <span data-ttu-id="0046a-464">**Microsoft.Owin.Security.MicrosoftAccount** Microsoft hesaplarını kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-464">**Microsoft.Owin.Security.MicrosoftAccount** Enables authentication using Microsoft accounts.</span></span>
- <span data-ttu-id="0046a-465">**Microsoft.Owin.Security.OAuth**.</span><span class="sxs-lookup"><span data-stu-id="0046a-465">**Microsoft.Owin.Security.OAuth**.</span></span> <span data-ttu-id="0046a-466">Taşıyıcı belirteç kimlik doğrulaması için bir OAuth yetkilendirme sunucusu yanı sıra ara yazılım sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-466">Provides an OAuth authorization server as well as middleware for authenticating bearer tokens.</span></span>
- <span data-ttu-id="0046a-467">**Microsoft.Owin.Security.Twitter** Twitter'ın OAuth tabanlı hizmetini kullanarak kimlik doğrulamayı etkinleştirir.</span><span class="sxs-lookup"><span data-stu-id="0046a-467">**Microsoft.Owin.Security.Twitter** Enables authentication using Twitter's OAuth-based service.</span></span>

<span data-ttu-id="0046a-468">Bu sürüm ayrıca içerir `Microsoft.Owin.Cors` çıkış noktaları arası HTTP isteklerini işlemek için bir ara yazılım içeren paket.</span><span class="sxs-lookup"><span data-stu-id="0046a-468">This release also includes the `Microsoft.Owin.Cors` package, which contains middleware for processing cross-origin HTTP requests.</span></span>

> [!NOTE]
> <span data-ttu-id="0046a-469">Visual Studio 2013'ün son sürümde JWT imzalamak için destek kaldırılmıştır.</span><span class="sxs-lookup"><span data-stu-id="0046a-469">Support for JWT signing has been removed in the final version of Visual Studio 2013.</span></span>

<a id="ef6"></a>
## <a name="entity-framework-6"></a><span data-ttu-id="0046a-470">Entity Framework 6</span><span class="sxs-lookup"><span data-stu-id="0046a-470">Entity Framework 6</span></span>

<span data-ttu-id="0046a-471">Yeni özellikler ve Entity Framework 6 diğer değişiklikler listesi için bkz. [Entity Framework sürüm geçmişi](https://msdn.com/data/jj574253).</span><span class="sxs-lookup"><span data-stu-id="0046a-471">For a list of new features and other changes in Entity Framework 6, see [Entity Framework Version History](https://msdn.com/data/jj574253).</span></span>

<a id="TOC14"></a>
## <a name="aspnet-razor-3"></a><span data-ttu-id="0046a-472">ASP.NET Razor 3</span><span class="sxs-lookup"><span data-stu-id="0046a-472">ASP.NET Razor 3</span></span>

<span data-ttu-id="0046a-473">ASP.NET Razor 3, aşağıdaki yeni özellikler içerir:</span><span class="sxs-lookup"><span data-stu-id="0046a-473">ASP.NET Razor 3 includes the following new features:</span></span>

- <span data-ttu-id="0046a-474">İçin sekmesinde düzenleme desteği.</span><span class="sxs-lookup"><span data-stu-id="0046a-474">Support for Tab editing.</span></span> <span data-ttu-id="0046a-475">Preivously, **belgeyi Biçimlendir** komutu, otomatik girintileme ve biçimlendirme Visual Studio'da otomatik çalışmadığı doğru kullanırken **sekmeleri tut** seçeneği.</span><span class="sxs-lookup"><span data-stu-id="0046a-475">Preivously, the **Format Document** command, auto indenting, and auto formatting in Visual Studio did not work correctly when using the **Keep Tabs** option.</span></span> <span data-ttu-id="0046a-476">Bu değişiklik, Visual Studio için sekmesinde biçimlendirme Razor kodu biçimlendirme düzeltir.</span><span class="sxs-lookup"><span data-stu-id="0046a-476">This change corrects Visual Studio formatting for Razor code for tab formatting.</span></span>
- <span data-ttu-id="0046a-477">Bağlantılar oluşturulurken URL yeniden yazma kuralları için destek.</span><span class="sxs-lookup"><span data-stu-id="0046a-477">Support for URL Rewrite rules when generating links.</span></span>
- <span data-ttu-id="0046a-478">Saydam güvenlik özniteliğine kaldırılması.</span><span class="sxs-lookup"><span data-stu-id="0046a-478">Removal of security transparent attribute.</span></span>
  > [!NOTE]
  > <span data-ttu-id="0046a-479">Bu bölünmesi farklıdır ve Razor 2 MVC5 veya MVC5 karşı derlenen derlemeler ile uyumlu olduğu sürece Razor 3 MVC4 ve önceki sürümleri uyumsuz hale getiriyor.</span><span class="sxs-lookup"><span data-stu-id="0046a-479">This is a breaking change, and makes Razor 3 incompatible with MVC4 and earlier, while Razor 2 is incompatible with MVC5 or assemblies compiled against MVC5.</span></span>

<span data-ttu-id="0046a-480">Yayın öncesi sürümlerinden Visual Studio 2013'te düzeltilen razor 3 sorunlara bulunabilir [burada](https://aspnetwebstack.codeplex.com/workitem/list/advanced?keyword=&status=Resolved%7cClosed&type=All&priority=All&release=All%7cv5.0%2bPreview%7cv5.0%2bRC%7cv5.0%2bRTM&assignedTo=All&component=Web%2bPages%252fRazor&reasonClosed=Fixed&sortField=LastUpdatedDate&sortDirection=Descending&page=0).</span><span class="sxs-lookup"><span data-stu-id="0046a-480">Razor 3 issues fixed in Visual Studio 2013 from pre-release versions can be found [here](https://aspnetwebstack.codeplex.com/workitem/list/advanced?keyword=&status=Resolved%7cClosed&type=All&priority=All&release=All%7cv5.0%2bPreview%7cv5.0%2bRC%7cv5.0%2bRTM&assignedTo=All&component=Web%2bPages%252fRazor&reasonClosed=Fixed&sortField=LastUpdatedDate&sortDirection=Descending&page=0).</span></span>

<a id="TOC15"></a>
## <a name="aspnet-app-suspend"></a><span data-ttu-id="0046a-481">ASP.NET uygulama askıya alma</span><span class="sxs-lookup"><span data-stu-id="0046a-481">ASP.NET App Suspend</span></span>

<span data-ttu-id="0046a-482">ASP.NET uygulama askıya alma çok sayıda ASP.NET siteleri tek bir makinede barındırma için ekonomik bir model ve kullanıcı deneyimini önemli ölçüde değişir .NET Framework 4.5.1 oyunun kurallarını değiştiren bir özelliğidir.</span><span class="sxs-lookup"><span data-stu-id="0046a-482">ASP.NET App Suspend is a game-changing feature in the .NET Framework 4.5.1 that radically changes the user experience and economic model for hosting large numbers of ASP.NET sites on a single machine.</span></span> <span data-ttu-id="0046a-483">Daha fazla bilgi için [ASP.NET uygulama askıya alma – hızlı yanıt veren paylaşılan .NET web barındırma](https://blogs.msdn.com/b/dotnet/archive/2013/10/09/asp-net-app-suspend-responsive-shared-net-web-hosting.aspx).</span><span class="sxs-lookup"><span data-stu-id="0046a-483">For more information, see [ASP.NET App Suspend – responsive shared .NET web hosting](https://blogs.msdn.com/b/dotnet/archive/2013/10/09/asp-net-app-suspend-responsive-shared-net-web-hosting.aspx).</span></span>

<a id="knownissues"></a>
## <a name="known-issues-and-breaking-changes"></a><span data-ttu-id="0046a-484">Bilinen sorunlar ve yeni değişiklikler</span><span class="sxs-lookup"><span data-stu-id="0046a-484">Known Issues and Breaking Changes</span></span>

<span data-ttu-id="0046a-485">Bu bölümde, Visual Studio 2013 için bilinen sorunlar ve ASP.NET ve Web Araçları'ndaki önemli değişiklikler açıklanmaktadır.</span><span class="sxs-lookup"><span data-stu-id="0046a-485">This section describes known issues and breaking changes in the ASP.NET and Web Tools for Visual Studio 2013.</span></span>

### <a name="nuget"></a><span data-ttu-id="0046a-486">NuGet</span><span class="sxs-lookup"><span data-stu-id="0046a-486">NuGet</span></span>

- <span data-ttu-id="0046a-487">[Yeni paket geri yükleme, Mono üzerinde çalışmaz, SLN dosyasını kullanırken](https://nuget.codeplex.com/workitem/3596) – bir gelecek nuget.exe indirmesinde düzeltilecektir ve [NuGet.CommandLine paket](http://www.nuget.org/packages/NuGet.CommandLine/) güncelleştirin.</span><span class="sxs-lookup"><span data-stu-id="0046a-487">[New package restore doesn't work on Mono when using SLN file](https://nuget.codeplex.com/workitem/3596) – will be fixed in an upcoming nuget.exe download and [NuGet.CommandLine package](http://www.nuget.org/packages/NuGet.CommandLine/) update.</span></span>
- <span data-ttu-id="0046a-488">[Yeni paket geri yükleme, Wix projeleriyle çalışmıyor](https://nuget.codeplex.com/workitem/3598) – bir gelecek nuget.exe indirmesinde düzeltilecektir ve [NuGet.CommandLine paket](http://www.nuget.org/packages/NuGet.CommandLine/) güncelleştirin.</span><span class="sxs-lookup"><span data-stu-id="0046a-488">[New package restore doesn't work with Wix projects](https://nuget.codeplex.com/workitem/3598) – will be fixed in an upcoming nuget.exe download and [NuGet.CommandLine package](http://www.nuget.org/packages/NuGet.CommandLine/) update.</span></span>
- <span data-ttu-id="0046a-489">[Otomatik paket geri yükleme projelerin bir çözüm klasörü altında çalışmaz](https://nuget.codeplex.com/workitem/3625) – NuGet 2.8 düzeltilecektir.</span><span class="sxs-lookup"><span data-stu-id="0046a-489">[Automatic Package restore doesn't work for projects under a solution folder](https://nuget.codeplex.com/workitem/3625) – will be fixed in NuGet 2.8.</span></span>

### <a name="aspnet-web-api"></a><span data-ttu-id="0046a-490">ASP.NET Web API</span><span class="sxs-lookup"><span data-stu-id="0046a-490">ASP.NET Web API</span></span>

1. <span data-ttu-id="0046a-491">`ODataQueryOptions<T>.ApplyTo(IQueryable)` döndürmüyor `IQueryable<T>` için destek ekledik olarak her zaman `$select` ve `$expand`.</span><span class="sxs-lookup"><span data-stu-id="0046a-491">`ODataQueryOptions<T>.ApplyTo(IQueryable)` doesn't return `IQueryable<T>` always, as we added support for `$select` and `$expand`.</span></span>

    <span data-ttu-id="0046a-492">Önceki örneklerimizi `ODataQueryOptions<T>` her zaman Integer dönüş değeri `ApplyTo` için `IQueryable<T>`.</span><span class="sxs-lookup"><span data-stu-id="0046a-492">Our earlier samples for `ODataQueryOptions<T>` always casted the return value from `ApplyTo` to `IQueryable<T>`.</span></span> <span data-ttu-id="0046a-493">Bu sorgu seçeneği için daha önce çalışan biz daha önce desteklenen (`$filter`, `$orderby`, `$skip`, `$top`) sorgu şeklini değiştirmeyin.</span><span class="sxs-lookup"><span data-stu-id="0046a-493">This worked earlier because the query options that we supported earlier (`$filter`, `$orderby`, `$skip`, `$top`) do not change the shape of the query.</span></span> <span data-ttu-id="0046a-494">Destekliyoruz göre `$select` ve `$expand` dönüş değeri `ApplyTo` değişmeyecek `IQueryable<T>` her zaman.</span><span class="sxs-lookup"><span data-stu-id="0046a-494">Now that we support `$select` and `$expand` the return value from `ApplyTo` will not be `IQueryable<T>` always.</span></span>

    [!code-csharp[Main](release-notes/samples/sample21.cs)]

    <span data-ttu-id="0046a-495">Daha önce'nden örnek kodu kullanıyorsanız, istemciyi değil gönderirseniz, çalışmaya devam eder `$select` ve `$expand`.</span><span class="sxs-lookup"><span data-stu-id="0046a-495">If you are using the sample code from earlier, it will continue working if the client does not send `$select` and `$expand`.</span></span> <span data-ttu-id="0046a-496">Ancak, desteklemek istediğiniz `$select` ve `$expand` için bu kodu değiştirmek zorunda.</span><span class="sxs-lookup"><span data-stu-id="0046a-496">However, if you wish to support `$select` and `$expand` you have to change that code to this.</span></span>

    [!code-csharp[Main](release-notes/samples/sample22.cs)]
2. <span data-ttu-id="0046a-497">**Toplu istek sırasında Request.Url veya RequestContext.Url null**</span><span class="sxs-lookup"><span data-stu-id="0046a-497">**Request.Url or RequestContext.Url is null during a batch request**</span></span>

    <span data-ttu-id="0046a-498">Bir toplu işlem senaryosunda **UrlHelper** den erişildiğinde null **Request.Url** veya **RequestContext.Url**.</span><span class="sxs-lookup"><span data-stu-id="0046a-498">In a batching scenario, **UrlHelper** is null when accessed from **Request.Url** or **RequestContext.Url**.</span></span>

    <span data-ttu-id="0046a-499">Bu sorun şu anda burada izlenir: [BatchRequestContext.Url isteği toplu işleme için null](http://aspnetwebstack.codeplex.com/workitem/1301).</span><span class="sxs-lookup"><span data-stu-id="0046a-499">This issue is currently tracked here: [BatchRequestContext.Url is null for batching request](http://aspnetwebstack.codeplex.com/workitem/1301).</span></span>

    <span data-ttu-id="0046a-500">Yeni bir örneğini oluşturmak için bu sorunun geçici çözümü olan **UrlHelper**, aşağıdaki örnekte olduğu gibi:</span><span class="sxs-lookup"><span data-stu-id="0046a-500">The workaround for this issue is to create a new instance of **UrlHelper**, as in the following example:</span></span>

    <span data-ttu-id="0046a-501">**UrlHelper yeni bir örneğini oluşturma**</span><span class="sxs-lookup"><span data-stu-id="0046a-501">**Creating a new instance of UrlHelper**</span></span>

    [!code-csharp[Main](release-notes/samples/sample23.cs)]

### <a name="aspnet-mvc"></a><span data-ttu-id="0046a-502">ASP.NET MVC</span><span class="sxs-lookup"><span data-stu-id="0046a-502">ASP.NET MVC</span></span>

1. <span data-ttu-id="0046a-503">Görünüm veri göndermek zaman AntiForgerToken doğrulama yapmak görünümler varsa MVC5 ve OrgAuth, kullanırken, şu hatayı arasında gelebilir:</span><span class="sxs-lookup"><span data-stu-id="0046a-503">When using MVC5 and OrgAuth, if you have views which do AntiForgerToken validation, you might come across the following error when you post data to the view:</span></span>

    <span data-ttu-id="0046a-504">**Hata**:</span><span class="sxs-lookup"><span data-stu-id="0046a-504">**Error**:</span></span>

    <span data-ttu-id="0046a-505">*'/' Uygulamasında sunucu hatası.*</span><span class="sxs-lookup"><span data-stu-id="0046a-505">*Server Error in '/' Application.*</span></span>

    <span data-ttu-id="0046a-506"><em>Bir talep türü '<http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier>'veya'<http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/identityprovider>' üzerinde sağlanan Claimsıdentity yoktu. Talep tabanlı kimlik doğrulaması ile sahteciliğe karşı koruma belirteci desteğini etkinleştirmek için lütfen yapılandırılan talep sağlayıcısı hem de bu talepler ürettiği Claimsıdentity örneklerinde sağladığını doğrulayın. Yapılandırılmış Talep sağlayıcı benzersiz bir tanımlayıcı olarak bunun yerine farklı talep türü kullanıyorsa, statik AntiForgeryConfig.UniqueClaimTypeIdentifier özelliği ayarlanarak yapılandırılabilir.</em></span><span class="sxs-lookup"><span data-stu-id="0046a-506"><em>A claim of type '<http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier>' or '<http://schemas.microsoft.com/accesscontrolservice/2010/07/claims/identityprovider>' was not present on the provided ClaimsIdentity. To enable anti-forgery token support with claims-based authentication, please verify that the configured claims provider is providing both of these claims on the ClaimsIdentity instances it generates. If the configured claims provider instead uses a different claim type as a unique identifier, it can be configured by setting the static property AntiForgeryConfig.UniqueClaimTypeIdentifier.</em></span></span>

    <span data-ttu-id="0046a-507">**Geçici çözüm**:</span><span class="sxs-lookup"><span data-stu-id="0046a-507">**Workaround**:</span></span>

    <span data-ttu-id="0046a-508">Sorunu gidermek için Global.asax dosyasında aşağıdaki satırı ekleyin:</span><span class="sxs-lookup"><span data-stu-id="0046a-508">Add the following line in Global.asax to fix it:</span></span>

    `AntiForgeryConfig.UniqueClaimTypeIdentifier = ClaimTypes.Name;`

    <span data-ttu-id="0046a-509">Bu, sonraki sürümde düzeltilecektir.</span><span class="sxs-lookup"><span data-stu-id="0046a-509">This will be fixed for the next release.</span></span>
2. <span data-ttu-id="0046a-510">MVC4 uygulaması için MVC5 yükseltme yaptıktan sonra çözümü oluşturun ve başlatın.</span><span class="sxs-lookup"><span data-stu-id="0046a-510">After upgrading an MVC4 app to MVC5, build the solution and launch it.</span></span> <span data-ttu-id="0046a-511">Şu hatayı göreceksiniz:</span><span class="sxs-lookup"><span data-stu-id="0046a-511">You should see the following error:</span></span>

    <span data-ttu-id="0046a-512">[A] [İçin B]System.Web.WebPages.Razor.Configuration.HostSection. System.Web.WebPages.Razor.Configuration.HostSection atanamaz</span><span class="sxs-lookup"><span data-stu-id="0046a-512">[A]System.Web.WebPages.Razor.Configuration.HostSection cannot be cast to [B]System.Web.WebPages.Razor.Configuration.HostSection.</span></span> <span data-ttu-id="0046a-513">Tür A kaynaklanan ' System.Web.WebPages.Razor, sürüm 2.0.0.0, Culture = neutral, PublicKeyToken = 31bf3856ad364e35 ' konumunda ' Default' bağlamında ' C:\windows\Microsoft.Net\assembly\GAC\_MSIL\System.Web.WebPages.Razor\ v4.0\_2.0.0.0\_\_31bf3856ad364e35\System.Web.WebPages.Razor.dll'.</span><span class="sxs-lookup"><span data-stu-id="0046a-513">Type A originates from 'System.Web.WebPages.Razor, Version=2.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35' in the context 'Default' at location 'C:\windows\Microsoft.Net\assembly\GAC\_MSIL\System.Web.WebPages.Razor\v4.0\_2.0.0.0\_\_31bf3856ad364e35\System.Web.WebPages.Razor.dll'.</span></span> <span data-ttu-id="0046a-514">Tür B kaynaklanan ' System.Web.WebPages.Razor, sürüm 3.0.0.0, Culture = neutral, PublicKeyToken = 31bf3856ad364e35 ' konumunda ' Default' bağlamında ' C:\Windows\Microsoft.NET\Framework\v4.0.30319\Temporary ASP.NET Files\root\6d05bbd0\ e8b5908e\assembly\dl3\c9cbca63\f8910382\_6273ce01\System.Web.WebPages.Razor.dll'.</span><span class="sxs-lookup"><span data-stu-id="0046a-514">Type B originates from 'System.Web.WebPages.Razor, Version=3.0.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35' in the context 'Default' at location 'C:\Windows\Microsoft.NET\Framework\v4.0.30319\Temporary ASP.NET Files\root\6d05bbd0\e8b5908e\assembly\dl3\c9cbca63\f8910382\_6273ce01\System.Web.WebPages.Razor.dll'.</span></span>

    <span data-ttu-id="0046a-515">Yukarıdaki hatayı düzeltmek için açık *tüm* (görünümleri klasöründe Devralınanlar da dahil) Web.config dosyalarından proje ve aşağıdakileri yapın:</span><span class="sxs-lookup"><span data-stu-id="0046a-515">To fix the above error, open *all* the Web.config files (including the ones in the Views folder) in your project and do the following:</span></span>

   1. <span data-ttu-id="0046a-516">Tüm oluşumlarını sürümü "4.0.0.0" "System.Web.Mvc" için "5.0.0.0" güncelleştirin.</span><span class="sxs-lookup"><span data-stu-id="0046a-516">Update all occurrences of version "4.0.0.0" of "System.Web.Mvc" to "5.0.0.0".</span></span>
   2. <span data-ttu-id="0046a-517">"System.Web.Helpers", "2.0.0.0" sürümünün tüm oluşumları güncelleştirme &quot;System.Web.WebPages&quot; ve &quot;System.Web.WebPages.Razor&quot; "3.0.0.0" için</span><span class="sxs-lookup"><span data-stu-id="0046a-517">Update all occurrences of version "2.0.0.0" of "System.Web.Helpers", &quot;System.Web.WebPages&quot; and &quot;System.Web.WebPages.Razor&quot; to "3.0.0.0"</span></span>

      <span data-ttu-id="0046a-518">Örneğin, yukarıdaki değişiklikler yaptıktan sonra derleme bağlamaları şu şekilde görünmelidir:</span><span class="sxs-lookup"><span data-stu-id="0046a-518">For example, after you make the above changes, the assembly bindings should look like this:</span></span>

      [!code-xml[Main](release-notes/samples/sample24.xml)]

      <span data-ttu-id="0046a-519">MVC 5 MVC 4 projelerini yükseltme hakkında daha fazla bilgi için bkz: [bir ASP.NET MVC 4 ve Web API projelerini ASP.NET MVC 5 ve Web API 2'ye yükseltme](../../../mvc/overview/releases/how-to-upgrade-an-aspnet-mvc-4-and-web-api-project-to-aspnet-mvc-5-and-web-api-2.md).</span><span class="sxs-lookup"><span data-stu-id="0046a-519">For information on upgrading MVC 4 projects to MVC 5, see [How to Upgrade an ASP.NET MVC 4 and Web API Project to ASP.NET MVC 5 and Web API 2](../../../mvc/overview/releases/how-to-upgrade-an-aspnet-mvc-4-and-web-api-project-to-aspnet-mvc-5-and-web-api-2.md).</span></span>
3. <span data-ttu-id="0046a-520">İstemci tarafı doğrulama jQuery örtük doğrulaması ile kullanırken, doğrulama iletisi bazen türüne sahip bir HTML input öğesi için yanlış = 'number'.</span><span class="sxs-lookup"><span data-stu-id="0046a-520">When using client-side validation with jQuery Unobtrusive Validation, the validation message is sometimes incorrect for an HTML input element with type='number'.</span></span> <span data-ttu-id="0046a-521">Doğrulama hatası için gerekli bir değer ("Yaş alan gerekiyor") dataValues geçerli bir sayı gerekli olduğunu doğru iletisi yerine girildiğinde gösterilir.</span><span class="sxs-lookup"><span data-stu-id="0046a-521">The validation error for a required value ("The Age field is required") is shown when an invalid number is entered instead of the correct message that a valid number is required.</span></span>

    <span data-ttu-id="0046a-522">Bu sorun için bir model oluşturma ve düzenleme görünümlerde bir tamsayı özelliği ile iskele kurulmuş kodu ile yaygın olarak bulunur.</span><span class="sxs-lookup"><span data-stu-id="0046a-522">This issue is commonly found with scaffolded code for a model with an integer property on the Create and Edit views.</span></span>

    <span data-ttu-id="0046a-523">Bu sorunu çözmek için gelen Düzenleyicisi Yardımcısı değiştirin:</span><span class="sxs-lookup"><span data-stu-id="0046a-523">To work around this issue, change the editor helper from:</span></span>

    `@Html.EditorFor(person => person.Age)`

    <span data-ttu-id="0046a-524">Hedef:</span><span class="sxs-lookup"><span data-stu-id="0046a-524">To:</span></span>

    `@Html.TextBoxFor(person => person.Age)`
4. <span data-ttu-id="0046a-525">ASP.NET MVC 5 kısmi güven artık destekler.</span><span class="sxs-lookup"><span data-stu-id="0046a-525">ASP.NET MVC 5 no longer supports partial trust.</span></span> <span data-ttu-id="0046a-526">MVC veya Webapı ikili dosyalar için bağlama projeleri kaldırmalısınız [SecurityTransparent](https://msdn.microsoft.com/library/system.security.securitytransparentattribute.aspx) özniteliği ve [AllowPartiallyTrustedCallers](https://msdn.microsoft.com/library/system.security.allowpartiallytrustedcallersattribute.aspx) özniteliği.</span><span class="sxs-lookup"><span data-stu-id="0046a-526">Projects linking to the MVC or WebAPI binaries should remove the [SecurityTransparent](https://msdn.microsoft.com/library/system.security.securitytransparentattribute.aspx) attribute and the [AllowPartiallyTrustedCallers](https://msdn.microsoft.com/library/system.security.allowpartiallytrustedcallersattribute.aspx) attribute.</span></span> <span data-ttu-id="0046a-527">Bu öznitelikleri kaldırma, aşağıdaki gibi derleyici hatalarını ortadan kaldırır.</span><span class="sxs-lookup"><span data-stu-id="0046a-527">Removing these attributes will eliminate compiler errors such as the following.</span></span>

    `Attempt by security transparent method ‘MyComponent' to access security critical type 'System.Web.Mvc.MvcHtmlString' failed. Assembly 'PagedList.Mvc, Version=4.3.0.0, Culture=neutral, PublicKeyToken=abbb863e9397c5e1' is marked with the AllowPartiallyTrustedCallersAttribute, and uses the level 2 security transparency model. Level 2 transparency causes all methods in AllowPartiallyTrustedCallers assemblies to become security transparent by default, which may be the cause of this exception.`

    > <span data-ttu-id="0046a-528">Not, bir yan etkisi Bu, aynı uygulamada 4.0 ve 5.0 derlemeleri kullanamazsınız.</span><span class="sxs-lookup"><span data-stu-id="0046a-528">Note, as a side effect of this you cannot use 4.0 and 5.0 assemblies in the same application.</span></span> <span data-ttu-id="0046a-529">Tümünü 5.0 güncelleştirmeniz gerekir.</span><span class="sxs-lookup"><span data-stu-id="0046a-529">You need to update all of them to 5.0.</span></span>

### <a name="spa-template-with-facebook-authorization-may-cause-instability-in-ie-while-the-web-site-is-hosted-in-intranet-zone"></a><span data-ttu-id="0046a-530">Web sitesi intranet bölgesinde barındırılıyor olsa Facebook yetkilendirme SPA şablonuyla IE'de kararsız hale gelmesine neden</span><span class="sxs-lookup"><span data-stu-id="0046a-530">SPA Template with Facebook authorization may cause instability in IE while the web site is hosted in intranet zone</span></span>

<span data-ttu-id="0046a-531">SPA şablon Facebook dış oturum sağlar.</span><span class="sxs-lookup"><span data-stu-id="0046a-531">The SPA template provides external log in with Facebook.</span></span> <span data-ttu-id="0046a-532">Oturum açma, şablonla oluşturulan projeyi yerel olarak çalışırken, IE çökmesine neden olabilir.</span><span class="sxs-lookup"><span data-stu-id="0046a-532">When the project created with the template is running locally, signing in may cause IE to crash.</span></span>

<span data-ttu-id="0046a-533">Çözüm:</span><span class="sxs-lookup"><span data-stu-id="0046a-533">Solution:</span></span>

1. <span data-ttu-id="0046a-534">İnternet bölgesi web sitesinde barındırmak; veya</span><span class="sxs-lookup"><span data-stu-id="0046a-534">Host the web site in internet zone; or</span></span>

2. <span data-ttu-id="0046a-535">Senaryo, IE dışındaki bir tarayıcıda test edin.</span><span class="sxs-lookup"><span data-stu-id="0046a-535">Test the scenario in a browser other than IE.</span></span>

### <a name="web-forms-scaffolding"></a><span data-ttu-id="0046a-536">Yapı İskelesi Web formları</span><span class="sxs-lookup"><span data-stu-id="0046a-536">Web Forms Scaffolding</span></span>

<span data-ttu-id="0046a-537">Web Forms yapı İskelesi VS2013 ' kaldırıldı ve Visual Studio için gelecekte yapılacak bir güncelleştirmenin kullanıma sunulacak.</span><span class="sxs-lookup"><span data-stu-id="0046a-537">Web Forms Scaffolding has been removed from VS2013 and will be available in a future update to Visual Studio.</span></span> <span data-ttu-id="0046a-538">Ancak bir Web Forms projesindeki yapı iskelesi MVC bağımlılıkları ekleyerek ve yapı iskelesi MVC için oluşturma kullanmaya devam edebilirsiniz.</span><span class="sxs-lookup"><span data-stu-id="0046a-538">However, you can still use scaffolding within a Web Forms project by adding MVC dependencies and generating scaffolding for MVC.</span></span> <span data-ttu-id="0046a-539">WebForms ve MVC proje içerir.</span><span class="sxs-lookup"><span data-stu-id="0046a-539">Your project will contain a combination of Web Forms and MVC.</span></span>

<span data-ttu-id="0046a-540">MVC Web Forms projenize eklemek için yeni iskele kurulmuş öğe ekleme ve seçin **MVC 5 bağımlılıkları**.</span><span class="sxs-lookup"><span data-stu-id="0046a-540">To add MVC to your Web Forms project, add a new scaffolded item and select **MVC 5 Dependencies**.</span></span> <span data-ttu-id="0046a-541">En az ya da tam tüm betikler gibi içerik dosyaları ihtiyacınız olup olmadığına bağlı olarak seçin.</span><span class="sxs-lookup"><span data-stu-id="0046a-541">Select either Minimal or Full depending on whether you need all of the content files, such as scripts.</span></span> <span data-ttu-id="0046a-542">Ardından, projenizde görünümleri ve denetleyici oluşturur, MVC için iskele kurulmuş öğe ekleyin.</span><span class="sxs-lookup"><span data-stu-id="0046a-542">Then, add a scaffolded item for MVC, which will create views and a controller in your project.</span></span>

### <a name="mvc-and-web-api-scaffolding---http-404-not-found-error"></a><span data-ttu-id="0046a-543">MVC ve Web APİ'si yapı iskelesini - HTTP 404 bulunamadı hatası</span><span class="sxs-lookup"><span data-stu-id="0046a-543">MVC and Web API Scaffolding - HTTP 404, Not Found error</span></span>

<span data-ttu-id="0046a-544">İskele kurulmuş öğe projeye eklenirken bir hatayla karşılaştı, projenizi tutarsız bir durumda bırakılır mümkündür.</span><span class="sxs-lookup"><span data-stu-id="0046a-544">If an error is encountered when adding a scaffolded item to a project, it is possible your project will be left in an inconsistent state.</span></span> <span data-ttu-id="0046a-545">Bazı yapı iskelesi yapılacak değişiklikler geri alınır ancak yüklü NuGet paketleri gibi diğer değişiklikleri geri alınmaz.</span><span class="sxs-lookup"><span data-stu-id="0046a-545">Some of the changes made be scaffolding will be rolled back but other changes, such as the installed NuGet packages, will not be rolled back.</span></span> <span data-ttu-id="0046a-546">Yönlendirme yapılandırma değişikliklerini geri giderek iskele kurulmuş öğe olduğunda kullanıcıların bu HTTP 404 hatası alırsınız.</span><span class="sxs-lookup"><span data-stu-id="0046a-546">If the routing configuration changes are rolled back, users will receive an HTTP 404 error when navigating to scaffolded items.</span></span>

<span data-ttu-id="0046a-547">Geçici çözüm:</span><span class="sxs-lookup"><span data-stu-id="0046a-547">Workaround:</span></span>

- <span data-ttu-id="0046a-548">MVC için bu hatayı düzeltmek için yeni iskele kurulmuş öğe ekleme ve MVC 5 bağımlılıkları seçin (en az veya tam).</span><span class="sxs-lookup"><span data-stu-id="0046a-548">To fix this error for MVC, add a new scaffolded item and select MVC 5 Dependencies (either Minimal or Full).</span></span> <span data-ttu-id="0046a-549">Bu işlem tüm gerekli değişiklikleri projenize ekleyin.</span><span class="sxs-lookup"><span data-stu-id="0046a-549">This process will add all of the required changes to your project.</span></span>
- <span data-ttu-id="0046a-550">Web API'si için bu hatayı düzeltmek için:</span><span class="sxs-lookup"><span data-stu-id="0046a-550">To fix this error for Web API:</span></span>

  1. <span data-ttu-id="0046a-551">WebApiConfig sınıfı projenize ekleyin.</span><span class="sxs-lookup"><span data-stu-id="0046a-551">Add the WebApiConfig class to your project.</span></span>

      [!code-csharp[Main](release-notes/samples/sample25.cs)]

      [!code-vb[Main](release-notes/samples/sample26.vb)]
  2. <span data-ttu-id="0046a-552">WebApiConfig.Register yapılandırın\_yöntemi Global.asax dosyasında şu şekilde başlatın:</span><span class="sxs-lookup"><span data-stu-id="0046a-552">Configure WebApiConfig.Register in the Application\_Start method in Global.asax as follows:</span></span>
