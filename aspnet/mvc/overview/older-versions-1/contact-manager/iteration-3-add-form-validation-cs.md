...
[Kodu indirme](iteration-3-add-form-validation-cs/_static/contactmanager_3_cs1.zip)
...
**Şekil 01**: doğrulama formuyla ([tam boyutlu görüntüyü görüntülemek için tıklatın](iteration-3-add-form-validation-cs/_static/image2.png))
...
Hemen HTML formu görünür Html.ValidationSummary() yardımcı yöntemine çağrı dikkat edin. Doğrulama hata iletisi varsa, bu yöntem doğrulama iletileri madde işaretli listede görüntüler.

Bildirim, ayrıca, her bir form alanından yanında görüntülenen Html.ValidationMessage() çağrıları. ValidationMessage() yardımcı tek tek doğrulama hata iletisi görüntüler. Bir doğrulama hatası olduğunda listeleme 1 söz konusu olduğunda, bir yıldız işareti görüntülenir.

Son olarak, Yardımcısı tarafından görüntülenen özelliği ile ilişkilendirilen bir doğrulama hatası olduğunda Html.TextBox() yardımcı otomatik olarak bir geçişli stil sayfası sınıfı oluşturur. Adlı bir sınıf Html.TextBox() Yardımcısı işler **giriş doğrulama hatası**.
...
**2 - Controllers\ContactController.cs (Oluştur doğrulama ile) listeleme**
