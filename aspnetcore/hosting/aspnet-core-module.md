...
*Web.config* aşağıdaki örnek olduğu için bir [müstakil dağıtım](https://docs.microsoft.com/dotnet/articles/core/deploying/#self-contained-deployments-scd) için [Azure App Service](https://azure.microsoft.com/services/app-service/). Daha fazla bilgi için bkz: [IIS yayımlama](xref:publishing/iis). Bkz: [alt uygulama yapılandırma](xref:publishing/iis#configuration-of-sub-applications) yapılandırmasındaki ilgili önemli bir not için *web.config* alt uygulamalarında dosyaları.
...
ASP.NET çekirdeği Modülü Yükleyicisi ayrıcalıklarıyla çalıştırır **sistem** hesabı. IIS paylaşılan yapılandırması tarafından kullanılan paylaşım yolu için izin yerel sistem hesabını değiştirme çünkü Yükleyici bir erişim reddedildi hatası modülü ayarlarını yapılandırmak çalışırken karşılaşır  *applicationHost.config* paylaşımda.
