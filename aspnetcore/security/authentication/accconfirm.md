...
Bkz: [bu PDF dosyası](https://webpifeed.blob.core.windows.net/webpifeed/Partners/asp.net_repo_pdf_1-16-18.pdf) ASP.NET Core 1.1 sürümü için.
...
## <a name="create-and-test-a-web-app-with-authentication"></a>Oluşturma ve kimlik doğrulaması ile bir web uygulamasını test etme
...
Uygulamayı çalıştırın, seçin **kaydetme** bağlamak ve bir kullanıcı kaydı. Kaydedildikten sonra yönlendirilirsiniz için `/Identity/Account/RegisterConfirmation` e-posta onayı benzetimini yapmak için bir bağlantı içeren bir sayfa:

* Seçin `Click here to confirm your account` bağlantı.
* Seçin **oturum açma** bağlamak ve oturum açma ile aynı kimlik bilgileri.
* Seçin `Hello YourEmail@provider.com!` için yeniden yönlendiren bir bağlantı `/Identity/Account/Manage/PersonalData` sayfası.
* Seçin **kişisel verileri** sekmesinde solda ve ardından **Sil**.

### <a name="configure-an-email-provider"></a>Bir e-posta sağlayıcısı yapılandırma

Bu öğreticide [SendGrid](https://sendgrid.com) e-posta göndermek için kullanılır. SendGrid hesabı ve e-posta göndermek için anahtar ihtiyacınız var. Diğer e-posta sağlayıcılarının kullanabilirsiniz. E-posta göndermek için SendGrid veya başka bir e-posta hizmeti kullanmanızı öneririz. SMTP güvenli ve doğru bir şekilde ayarlamak zordur.
...
**En iyi güvenlik uygulaması** için **değil** test ve geliştirme üretim gizli dizileri kullanın. Uygulamayı Azure'a yayımlama, SendGrid gizli dizileri Azure Web uygulaması Portalı'ndaki uygulama ayarları olarak ayarlayın. Yapılandırma sistemi ortam değişkenlerinden anahtarları okumak için ayarlanır.
