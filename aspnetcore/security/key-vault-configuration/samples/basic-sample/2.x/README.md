# <a name="key-vault-configuration-provider-sample-application-aspnet-core-2x"></a>Anahtar kasası yapılandırma sağlayıcısı örnek uygulaması (ASP.NET Core 2.x)

Bu örnek ASP.NET Core için Azure anahtar kasası yapılandırma sağlayıcısı kullanımını göstermektedir 2.x. ASP.NET Core 1.x örnek için bkz: [anahtar kasası yapılandırma sağlayıcısı örnek uygulaması (ASP.NET Core 1.x)](https://github.com/aspnet/Docs/tree/master/aspnetcore/security/key-vault-configuration/samples/basic-sample/1.x).
