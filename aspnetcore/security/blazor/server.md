...
* Uygulamaya genel bir uç nokta gibi, JavaScript 'e sunulan tüm .NET metodunu değerlendirin.
  ...
  * Statik ve örnek yöntemlerinin JavaScript istemcilerine sunutabileceğiniz hesaba sahip olun. Tasarım, uygun kısıtlamalarla durum paylaşma için çağrı yaptığı müddetçe, oturumlar arasında durum paylaşmaktan kaçının.
    * İlk olarak bağımlılık ekleme ( `DotNetReference` dı) aracılığıyla oluşturulan nesneler aracılığıyla kullanıma sunulan örnek yöntemleri için nesnelerin kapsamlı nesneler olarak kaydedilmesi gerekir. Bu, Blazor Server uygulamasının kullandığı tüm DI Hizmetleri için geçerlidir.
...
Olaylar bir Blazor sunucu uygulamasına giriş noktası sağlar. Web Apps 'teki uç noktaları koruma için aynı kurallar, Blazor Server uygulamalarındaki olay işleme için geçerlidir. Kötü amaçlı bir istemci, istediği verileri bir olay için yük olarak gönderebilirler.
...
ASP.NET Core uygulamalarının güvenliğini sağlamaya yönelik kılavuzluk, Blazor Server uygulamaları için geçerlidir ve aşağıdaki bölümlerde ele alınmıştır:
...
Blazor sunucusu, istemci ve sunucu arasındaki iletişim için SignalR kullanır. Blazor sunucusu normalde, genellikle WebSockets olan SignalR 'nin anlaşma yaptığı aktarımı kullanır.
...
Blazor Server Framework, önceki tehditlere karşı korumak için gereken adımları alır:
...
Çapraz kaynak saldırıları, sunucuya yönelik bir eylem gerçekleştiren farklı bir kaynaktan gelen bir istemciyi içerir. Kötü amaçlı eylem, genellikle bir GET isteği veya bir form GÖNDERISINI (siteler arası Istek sahteciliği, CSRF), ancak kötü amaçlı bir WebSocket açmak da mümkündür. Blazor Server uygulamaları, [hub protokolünü kullanan diğer bir SignalR uygulamasının aynısını](xref:signalr/security)sunar:

* Blazor sunucu uygulamalarına ek ölçüler alınana kadar, kaynak dışı erişilebilir. Çapraz kaynak erişimini devre dışı bırakmak için, işlem hattında CORS ana hattını ekleyerek ve Blazor uç noktası meta verilerine ekleyerek ve izin verilen `DisableCorsAttribute` kaynaklar kümesini, [çıkış noktaları için SignalR 'yi yapılandırarak, bu uç noktada CORS 'yi devre dışı bırakın paylaşma](xref:signalr/security#cross-origin-resource-sharing).
* CORS etkinse, CORS yapılandırmasına bağlı olarak uygulamayı korumak için ek adımlar gerekebilir. CORS genel olarak etkinleştirilmişse, Blazor sunucu hub 'ı çağrıldıktan `DisableCorsAttribute` `hub.MapBlazorHub()`sonra meta veri noktası meta verilerine ekleyerek CORS devre dışı bırakılabilir.
