...
* [ASP.NET Core](https://www.microsoft.com/net/download/core) (çalışma zamanı ve araçları)
...
3. Seçin **ASP.NET çekirdek Web uygulaması** proje şablonu. Altında görünür **yüklü** > **şablonları** > **Visual C#** > **.NET Core**. Proje adı `SampleWebAppDemo`. Seçin **yeni Git deposunun oluşturma** seçeneğini ve tıklayın **Tamam**.

   ![Yeni Proje iletişim kutusu](azure-continuous-deployment/_static/01-new-project.png)

4. İçinde **yeni ASP.NET Core projesi** iletişim kutusunda, ASP.NET Core seçin **boş** şablonu, ardından **Tamam**.
...
2. Tıklatın **uygulama hizmetleri** Azure aboneliğinizle ilişkili uygulama hizmetleri listesini görüntülemek için.

3. Bu öğreticinin önceki bölümde oluşturduğunuz web uygulaması seçin.

4. İçinde **dağıtım** dikey penceresinde, select **dağıtım seçenekleri** > **Kaynağı Seç** > **yerel Git deposu**.

   ![Ayarlar dikey: dağıtım kaynak dikey: kaynak dikey seçin](azure-continuous-deployment/_static/deployment-options.png)

5. **Tamam**'ı tıklatın.

6. Daha önce bir web uygulaması veya diğer App Service uygulama yayımlamak için dağıtım kimlik bilgileri ayarlamadıysanız, bunları şimdi kurun:
...
7. İçinde **Web uygulaması** dikey penceresinde tıklatın **ayarları** > **özellikleri**. İçin dağıtacaksınız uzak Git deposu URL'sini altında gösterilen **GIT URL'si**.

8. Kopya **GIT URL'si** öğreticide daha sonra kullanmak için değer.
...
1. İçinde [Azure Portal](https://portal.azure.com), web uygulamanızı seçin. Ardından, seçin **dağıtım** > **dağıtım seçenekleri**.
