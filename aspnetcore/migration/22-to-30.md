...
@No__t_0 `HostBuilder` olarak en önemli değişiklik, [bağımlılık ekleme (dı)](xref:fundamentals/dependency-injection)içinde. @No__t_0 kullanırken, yalnızca `Startup` oluşturucusuna <xref:Microsoft.Extensions.Configuration.IConfiguration> ve <xref:Microsoft.AspNetCore.Hosting.IHostingEnvironment> ekleyebilirsiniz. @No__t_0 dı kısıtlamaları:
