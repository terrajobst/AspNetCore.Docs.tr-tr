...
* @No__t_0:
  ...
  * @No__t_0 öğesini kaldırır. Daha fazla bilgi için bu belgede [Işlem içi barındırma modeli](#in-process-hosting-model) konusuna bakın.
...
* @No__t_0 SDK 'Yı hedefleyen projeler `Microsoft.AspNetCore.App` çerçevesine örtülü olarak başvurur.
...
* @No__t_0 veya `Microsoft.NET.Sdk.Razor` SDK 'yi hedefleyen projeler, `Microsoft.AspNetCore.App` için açık bir `FrameworkReference` eklemeli:
...
* @No__t_1 `System.Net.HttpClient` için biçimlendirme ve içerik anlaşması desteği [Microsoft. Aspnet. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) NuGet paketi, `ReadAsAsync`, `PostJsonAsync` vb. gibi apı 'lerle `System.Net.HttpClient` yararlı genişletilebilirlik sağlar.
...
* `services.AddMvc`-1 `services.AddRazorPages` ' e bakın. bu belgede [MVC hizmeti kaydı](#mvc-service-registration) .
...
* `app.UseAuthorization` ' ın şablonlarına, sıra yetkilendirmesi ara yazılımı eklenmesi gerektiğini gösterecek şekilde eklenmiştir. Uygulama yetkilendirme kullanmıyorsa, `app.UseAuthorization` ' a çağrıyı güvenle kaldırabilirsiniz.
* `app.UseEndpoints`, bkz. [Razor Pages](#razor-pages) veya [geçişi başlatma.](#migrate-startupconfigure) bu belgedeki yapılandırma.
...
* @No__t_0 olarak 3,0 'de kullanılabilir.
...
* `GetTrailer` &ndash;, yanıttan istenen sondaki üstbilgiyi alır. @No__t_1 çağrılmadan önce `SupportsTrailers` kontrol edin veya istek sondaki üstbilgileri desteklemiyorsa bir <xref:System.NotSupportedException> meydana gelebilir.
...
@No__t_1 seçenekleri ile `AllowSynchronousIO` etkinleştirmenin yanı sıra, zaman uyumlu GÇ geçici bir risk azaltma olarak istek başına temelinde da geçersiz kılınabilir:
...
@No__t_0 uygulamalarla veya [Dispose](/dotnet/standard/garbage-collection/implementing-dispose)içinde zaman uyumlu API 'leri çağıran diğer akışlarla ilgili sorun yaşıyorsanız, bunun yerine yenı <xref:System.IO.Stream.DisposeAsync*> API 'sini çağırın.
...
* @No__t_1 çağırmak için `Startup.ConfigureServices` güncelleştirin.
...
@No__t_0 MVC senaryolarıyla ilgili üç yeni en üst düzey uzantı yöntemi mevcuttur. Şablonlar `AddMvc` yerine bu yeni yöntemleri kullanır. Ancak, `AddMvc` önceki sürümlerde olduğu gibi davranmaya devam eder.
 ...
 * @No__t_0 kullanın ve istenen yol adını geçirin.

Endpoint Routing, `IRouter` olarak aynı yol deseninin sözdizimini ve yol deseninin yazma özelliklerini destekler. Uç nokta yönlendirme `IRouteConstraint` destekler. Uç nokta yönlendirme `[Route]`, `[HttpGet]` ve diğer MVC yönlendirme özniteliklerini destekler.
...
* @No__t_0 veya `UseSignalR` `UseEndpoints` ile değiştirin.
...
* @No__t_0 `IWebHostEnvironment` ile değiştirin ve <xref:Microsoft.Extensions.Hosting?displayProperty=fullName> ad alanı için bir `using` açıklaması ekleyin.
* @No__t_0 <xref:Microsoft.Extensions.Hosting.IHostApplicationLifetime> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> ad alanı) ile değiştirin.
* @No__t_0 <xref:Microsoft.Extensions.Hosting.Environments> (<xref:Microsoft.Extensions.Hosting?displayProperty=fullName> ad alanı) ile değiştirin.
...
Sistem durumu denetimleri, genel ana bilgisayar ile Endpoint Routing kullanır. @No__t_0, uç nokta URL 'SI veya göreli yol ile Endpoint Builder üzerinde `MapHealthChecks` çağırın:
...
* @No__t_0 sınıfı CORS 'yi `[DisableCors]` özniteliğiyle devre dışı bırakır.
...
ASP.NET Core önceki sürümlerinde, yetkilendirme desteği `[Authorize]` özniteliği aracılığıyla sağlanmış. Yetkilendirme ara yazılımı kullanılamıyor. ASP.NET Core 3,0 ' de, yetkilendirme ara yazılımı gereklidir. ASP.NET Core yetkilendirmesi ara yazılımını (`UseAuthorization`) `UseAuthentication` ' den hemen sonra yerleştirmenizi öneririz. Yetkilendirme ara yazılımı, geçersiz kılınabilen varsayılan ilkeyle de yapılandırılabilir.
...
Alternatif olarak, tüm uç noktalar bir `FallbackPolicy` yapılandırarak `[Authorize]` veya `RequireAuthorization` olmadan yetkilendirme gerektirecek şekilde yapılandırılabilir. @No__t_0, `DefaultPolicy` farklıdır. @No__t_0 `[Authorize]` veya `RequireAuthorization` tetiklenir, ancak başka bir ilke ayarlanmamışsa `FallbackPolicy` tetiklenir. `FallbackPolicy` başlangıçta yetkilendirmesiz isteklere izin verecek şekilde yapılandırılır.
...
Yönlendirme şimdi yalnızca MVC 'den daha fazlasına destek içerdiğinden, bu yöntemlerin ne yapacaklarına açık bir şekilde sahip olması için terminoloji değişmiştir. @No__t_0 / `MapAreaControllerRoute` / `MapDefaultControllerRoute` gibi geleneksel yollar eklendikleri sırayla uygulanır. Önce daha belirli yollar (örneğin, bir alana yönelik yollar) yerleştirin.
...
* @No__t_0 sonekini belirten bağlantı oluşturma gerekiyor. Örneğin:
...
Uç nokta yönlendirme ile sistem durumu denetimlerini kullanmak için `MapHealthChecks` ekleyin. @No__t_0 yöntemi `UseHealthChecks` benzer bağımsız değişkenleri kabul eder. @No__t_1 üzerinden `MapHealthChecks` kullanmanın avantajı, yetkilendirme uygulayabilir ve eşleşen ilke üzerinde daha ayrıntılı denetime sahip olabilir.
...
@No__t_0 `HostBuilder` olarak en önemli değişiklik, [bağımlılık ekleme (dı)](xref:fundamentals/dependency-injection)içinde. @No__t_0 kullanırken, aşağıdakileri yalnızca `Startup` oluşturucusuna ekleyebilirsiniz:

* <xref:Microsoft.Extensions.Configuration.IConfiguration>
* `Microsoft.Extensions.Hosting.IHostEnvironment`
* <xref:Microsoft.AspNetCore.Hosting.IWebHostEnvironment>

@No__t_0 dı kısıtlamaları:
...
@No__t_0, serileştirici seçeneklerini ayarlamak için `AddJsonProtocol` çağırın.
...
@No__t_1 desteklenmeyen `Newtonsoft.Json` özellikler kullanıyorsanız, `Newtonsoft.Json` 'e geri dönebilirsiniz:
...
@No__t_0 örnek, geliştirme ortamlarında koşullu çalışma zamanı derlemesini olanaklı bir şekilde etkinleştiren bir örnek gösterir.
