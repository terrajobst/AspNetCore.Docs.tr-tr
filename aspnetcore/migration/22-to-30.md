...
ASP.NET Core [paylaşılan çerçeveye](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/) bağlı bir paket kullanan konsol uygulamalarının çerçeveye bağımlı derlemeleri aşağıdaki çalışma zamanı hatasını verebilir:
...
Uygulamanız Docker kullanıyorsa, ASP.NET Core 3,0 içeren bir temel görüntü kullanın. Örneğin, uygulamasında yönetilen Hyper-V konakları olarak eklemek için aşağıdaki yordamı kullanabilirsiniz.
...
* Veritabanı &ndash; sağlayıcıya özgü pakete başvurma hakkında daha fazla bilgi için, bkz. [veritabanı sağlayıcıları](/ef/core/providers/index). Entity Framework Core
...
* `System.Net.HttpClient` [Microsoft. Aspnet. WebApi. Client](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Client/) NuGet paketi için `System.Net.HttpClient` &ndash; biçimlendirme ve içerik anlaşması desteği `ReadAsAsync`, `PostJsonAsync` vb. gibi API 'lerle yararlı genişletilebilirlik sağlar.

* Razor görünümlerinin ve &ndash; sayfalarının çalışma zamanı derlemesi için Razor çalışma zamanı derleme desteği artık [Microsoft. aspnetcore. Mvc. Razor. runtimecompilation](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation)'un bir parçasıdır.

* MVC 'yi &ndash; `Newtonsoft.Json`kullanarakMVC desteği desteği artık [Microsoft. aspnetcore. Mvc. newtonsoftjson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)' ın bir parçasıdır. `Newtonsoft.Json`
...
Uygulamanız, jsonpatch `Newtonsoft.Json` veya dönüştürücüler gibi belirli `Newtonsoft.Json`bir özellik kullanıyorsa ya da bu şekilde [formatlara](xref:web-api/advanced/formatting) `Newtonsoft.Json`özgü türler varsa, tümleştirme gerektirebilir.
...
* Uygulama, veya `AuthorizePage` `UseAuthorization` `UseRouting` `UseEndpoints` `UseCors` `UseAuthentication`gibi kimlik doğrulama/yetkilendirme özelliklerini kullanıyorsa, bu çağrıyı ve önüne: sonra,, ancak önce: `[Authorize]`
...
`Startup.Configure` Aşağıdaki`MapRazorPages` yöntemde Razor Pages için destek ekler:
...
## <a name="mysqldataentityframeworkcore-currently-not-supported-on-aspnet-core-30"></a>MySql. Data. EntityFrameworkCore Şu anda ASP.NET Core 3,0 üzerinde desteklenmiyor
