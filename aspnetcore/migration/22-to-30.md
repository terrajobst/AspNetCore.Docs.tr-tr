...
* MVC `Newtonsoft.Json` (Json.NET) desteği, MVC 'nin `Newtonsoft.Json` kullanımı için &ndash; desteği artık [Microsoft. AspNetCore. Mvc. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.Mvc.NewtonsoftJson)' ın bir parçasıdır.
...
* `IWebHostEnvironment``IHostingEnvironment`, [Bu GitHub duyurusuna](https://github.com/dotnet/AspNetCore/issues/7749)bakın.
...
ASP.NET Core için varsayılan JSON serileştirici artık .NET Core 3,0 ' de yeni olan <xref:System.Text.Json>. Mümkün olduğunda `System.Text.Json` kullanmayı düşünün. Bu yüksek performanstır ve ek bir kitaplık bağımlılığı gerektirmez. Ancak `System.Text.Json` yeni olduğundan, şu anda uygulamanızın ihtiyaç duyacağı özellikleri eksik olabilir. Daha fazla bilgi için, bkz. [Newtonsoft. JSON 'Dan System. Text. JSON 'a geçiş](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to).

### <a name="use-newtonsoftjson-in-an-aspnet-core-30-opno-locsignalr-project"></a>ASP.NET Core 3,0 SignalR projesinde Newtonsoft. JSON kullanın

* [Microsoft. AspNetCore.SignalR'i yükler. Protocols. NewtonsoftJson](https://www.nuget.org/packages/Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson) NuGet paketi.

* İstemcisinde, `HubConnectionBuilder` örneğine `AddNewtonsoftJsonProtocol` yöntemi çağrısını zincirleyebilirsiniz:
...
* Sunucusunda, bir `AddNewtonsoftJsonProtocol` yöntemi çağrısını `Startup.ConfigureServices``AddSignalR` yöntemi çağrısına zincirleyebilirsiniz:
...
* `AddNewtonsoftJson`çağırmak için `Startup.ConfigureServices` güncelleştirin.
  ...
  * `AddControllers`
  ...
  `Newtonsoft.Json` ayarları `AddNewtonsoftJson`çağrısında ayarlanabilir:
...
* Uygulama `IRouter` uyguluyor veya `Route`devralırsa, değiştirme olarak [Dynamikro Utevaluestranseski](https://github.com/dotnet/AspNetCore.Docs/issues/12997) kullanın.
* Uygulama, URL 'Leri ayrıştırmak için MVC 'nin içindeki `RouteData.Routers` doğrudan erişirse, bunu [Linkparser. ParsePathByEndpointName](xref:Microsoft.AspNetCore.Routing.LinkParserEndpointNameAddressExtensions.ParsePathByEndpointName*)kullanımı ile değiştirebilirsiniz. 
...
Endpoint Routing kullanırken, `<xref:Microsoft.AspNetCore.Mvc.Authorization.AuthorizeFilter>` yapılandırılmasını ve yetkilendirme ara yazılımını bağlı olarak kullanmanızı öneririz.  Uygulama, MVC 'de Genel filtre olarak bir `AuthorizeFilter` kullanıyorsa, `AddAuthorization`çağrısında bir ilke sağlamak için kodu yeniden düzenlemeyi öneririz.
...
Yetkilendirme, belirli uç nokta sınıfları için de yapılandırılabilir. Aşağıdaki kod, yetkilendirme gerektiren belirli bir ilkeye sahip bir uygulamaya küresel `AuthorizeFilter` yapılandıran bir MVC uygulamasını dönüştürme örneğidir:
...
ASP.NET Core 3,0 ' de, ASP.NET Core MVC `Async` son ekini denetleyici eylem adlarından kaldırır. Hem yönlendirme hem de bağlantı oluşturma, bu yeni varsayılan değerinden etkilenir. Örnek:
...
* `Async` sonekini belirten bağlantı oluşturma gerekiyor. Örnek:
...
* Bağlantı oluşturma `Async` sonekin belirtilmesini gerektirmez. Örnek:
...
[System. Text. JSON içinde desteklenmeyen Newtonsoft. JSON özellikleri](/dotnet/standard/serialization/system-text-json-migrate-from-newtonsoft-how-to)kullanıyorsanız, `Newtonsoft.Json`' ye dönebilirsiniz. Bu makalenin önceki kısımlarında yer [ASP.NET Core 3,0 SignalR projede Newtonsoft. JSON kullanma](#use-newtonsoftjson-in-an-aspnet-core-30-signalr-project) konusuna bakın.
...
.NET Core 3,0 ve sonraki sürümlerde doğrulama sistemi, null olamayan parametrelere veya bir `[Required]` özniteliğine sahip olduklarından, bağlantılı özelliklere davranır. Daha fazla bilgi için bkz. [[Required] özniteliği](xref:mvc/models/validation?view=aspnetcore-3.1#required-attribute).
