...
* Uygulama, veya `[Authorize]`gibi `AuthorizePage` kimlik doğrulama/yetkilendirme özelliklerini kullanıyorsa `UseAuthentication` , çağrısını ve `UseAuthorization` **sonra** `UseRouting`da yerleştirin.
* Uygulama gibi [CORS](xref:security/cors) özelliklerini `[EnableCors]`kullanıyorsa, sonraki yere koyun `UseCors` .
