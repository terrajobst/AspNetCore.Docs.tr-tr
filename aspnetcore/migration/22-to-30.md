...
* `UseCors` Uygulama, veya `[Authorize]`gibi `AuthorizePage` kimlik doğrulama/yetkilendirme özelliklerini kullanıyorsa `UseAuthentication` , çağrıyı (CORS ara yazılımı kullanılıyorsa `UseAuthorization` ve sonrasında) ve **sonra** `UseRouting` da girin.
...
* İle `IHostingEnvironment` `Microsoft.Extensions.Hosting` değiştirin ve ad alanı `using` için bir ifade ekleyin. `IWebHostEnvironment`
...
Uygulama, MVC 'de Genel `AuthorizeFilter` bir filtre olarak kullanıyorsa, bir ilke `UseAuthorization` sağlamak için kodu yeniden düzenlemenizi öneririz.

Aşağıdaki örnekte, çağrıldığında tüm isteklere `UseAuthorization` uygulanacak özel bir ilke `HomeController` ve Kullanıcı, uygulamada oturum açmadan erişime izin verir:
...
Aşağıdaki örnekte, `UseAuthorization` yetkilendirmeyi varsayılan bir ilke olmadan işler, `/healthz` ancak sistem durumu denetimi uç noktası `admin` kullanıcının rolünde olmasını gerektirir: 
