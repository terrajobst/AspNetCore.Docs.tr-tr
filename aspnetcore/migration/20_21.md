...
* `Microsoft.AspNetCore.All` için paket başvurusunu `Microsoft.AspNetCore.App`bir paket başvurusuyla değiştirin. `Microsoft.AspNetCore.All`kaldırılan bağımlılıkları eklemeniz gerekebilir. Daha fazla bilgi için bkz. <xref:fundamentals/metapackage#migrate> ve <xref:fundamentals/metapackage-app>.
