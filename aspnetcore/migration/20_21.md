...
* Paket başvurusu "Sürüm" özniteliğini kaldırın `Microsoft.AspNetCore.App`. Projeleri `<Project Sdk="Microsoft.NET.Sdk.Web">` sürümü ayarlamanız gerekmez. Sürüm hedef framework tarafından örtük ve ASP.NET Core 2.1 çalışma biçimini en iyi eşleşme için seçilmiş. Daha fazla bilgi için [paylaşılan framework'ü hedefleyen projeler için kuralları](#rules-for-projects-targeting-the-shared-framework) bölümü.
...
A *paylaşılan çerçeve* derlemeleri kümesidir ( *.dll* dosyaları) uygulamanın klasörlerinde olmayan. Paylaşılan framework uygulamasını çalıştırmak için makinede yüklü olmalıdır. Daha fazla bilgi için [paylaşılan çerçeve](https://natemcmaster.com/blog/2018/08/29/netcore-primitives-2/).
...
Paylaşılan bir framework'ü hedefleyen projeler için bilinen sorunlar:
...
* Güncelleştirme  *\_LoginPartial.cshtml* dosyası:
  * Taşıma *sayfaları /\_LoginPartial.cshtml* için *sayfaları ve paylaşılan/\_LoginPartial.cshtml*.
  * Ekleme `asp-area="Identity"` form ve yer işareti bağlantıları için.
  * Güncelleştirme `<form />` öğesine `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
* İçinde *kimlik/alanlar/sayfaları/\_ViewStart.cshtml*, değiştirme `Layout = "/Pages/_Layout.cshtml"` için `Layout = "/Pages/Shared/_Layout.cshtml"`.
* *\_Layout.cshtml* dosyası aşağıdaki değişiklikleri vardır:
  ...
  * jQuery 2.2.0 3.3.1 için değiştirir.
