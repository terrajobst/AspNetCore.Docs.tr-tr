...
Benimsemeye [işlem içi IIS için barındırma modelini](xref:fundamentals/servers/aspnet-core-module#in-process-hosting-model), ekleme `<AspNetCoreHostingModel>` özellik değerini `InProcess` için bir `<PropertyGroup>` proje dosyasında:

```xml
<AspNetCoreHostingModel>InProcess</AspNetCoreHostingModel>
```

Daha fazla bilgi için bkz. <xref:host-and-deploy/aspnet-core-module#hosting-model>.
