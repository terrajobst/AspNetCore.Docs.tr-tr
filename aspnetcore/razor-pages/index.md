...
Bu belgede, örnekleri için `DbContext` içinde başlatılan [haline](https://github.com/aspnet/Docs/blob/master/aspnetcore/razor-pages/index/sample/RazorPagesContacts/Startup.cs#L15-L16) dosya.
...
Oluşturulan ad alanı için *Pages/Customers/Edit.cshtml* Razor sayfasını aynıdır `PageModel` sınıfı.
...
Kullanım `@page` yönergesi için:

* Bir sayfa için özel bir yol belirtin. Örneğin, rota hakkında sayfasına olarak ayarlanabilir `/Some/Other/Path` ile `@page "/Some/Other/Path"`.
* Parçaları bir sayfanın varsayılan yol ekleyin. Örneğin, bir "öğesi" kesimi sahip bir sayfanın varsayılan yol eklenebilir `@page "item"`.
* Bir sayfanın varsayılan rota için parametreleri ekleyin. Örneğin, bir ID parametresi `id`, içeren bir sayfa için gerekli olabilir `@page "{id}"`.

Bir tilde tarafından atanan bir kök göreli yol (`~`) yolunu başında desteklenir. Örneğin, `@page "~/Some/Other/Path"` aynı `@page "/Some/Other/Path"`.
...
[İndirme veya görüntüleme örnek kod](https://github.com/aspnet/Docs/tree/master/aspnetcore/razor-pages/index/sample).
