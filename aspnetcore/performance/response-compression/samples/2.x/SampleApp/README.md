...
Bu örnek, HTTP yanıtlarını sıkıştırmak için ASP.NET Core 2. x yanıt sıkıştırma ara yazılımı kullanımını gösterir. Örnek, metin ve görüntü yanıtları için gzip, Brotli ve özel sıkıştırma sağlayıcılarını gösterir ve sıkıştırma için bir MIME türünün nasıl ekleneceğini gösterir. ASP.NET Core 1. x örneği için bkz. [Yanıt sıkıştırması örnek uygulaması (ASP.NET Core 1. x)](https://github.com/aspnet/AspNetCore.Docs/tree/master/aspnetcore/performance/response-compression/samples/1.x).
